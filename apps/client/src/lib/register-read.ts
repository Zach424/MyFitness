export type RegisterReadFailureKind = 'offline' | 'refused' | 'service' | 'unknown'

export type RegisterReadPhase =
  'initial-loading' | 'ready' | 'refreshing' | 'initial-error' | 'stale'

type StatusCodeError = { statusCode?: unknown; errMsg?: unknown }

export const classifyRegisterReadFailure = (error: unknown): RegisterReadFailureKind => {
...
export const registerReadPhase = ({
  hasSnapshot,
  busy,
  hasFailure,
}: {
