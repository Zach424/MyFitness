...
待迁移总量：192 份（专题 18 份，历史 174 份）。

## 已受门禁保护的范围

- 六份活跃权威文档由 `myfitness-chinese-documentation/v2` 检查中文标题、正文占比和纯英文叙述行：项目状态、[产品简报](product/PRODUCT_BRIEF.md)、交付路线图、架构基线、用户身份运行手册与本索引。
...
已迁移的产品简报已转入活跃权威文档，不再计入本批次。退出条件：剩余三份文档分别完成中文正文迁移，并以结构签名或提交前后集合核对证明代码字面量、链接和数字未漂移；随后把本批次状态改为受保护。
