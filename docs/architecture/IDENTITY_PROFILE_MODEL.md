...
Status: implemented for the local iteration-003 boundary
...
| `consent_events`  | Immutable purpose/version acceptance facts | Unique user + purpose + version; no overwrite of acceptance time                     |
...
The current versions are `2026-07-18` for terms, privacy and health-data processing. A client must send the exact active versions. Revocation, renewed acceptance, export and deletion evidence are intentionally deferred to the privacy/operations iteration.
