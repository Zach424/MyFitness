...
训练同步投影按 `(started_at,created_at,id)` 升序输出顶层 workout；`UNIQUE (workout_id,position)`、`UNIQUE (exercise_id,position)` 与 `UNIQUE (workout_id,revision)` 分别保证动作、组和修订内部顺序。活动列表继续使用部分降序索引；迁移 0032 另建非部分 `(user_id,started_at,created_at,id)` 索引，服务包含软删除记录的便携导出全历史扫描。内部 `createWorkoutSetLayerSnapshot()` 在一次 active-owner、只读 repeatable-read 根事务中读取 workout 头，为每个头暴露一次性动作子流，再为每个动作暴露一次性 set 子流。set 查询通过 `workout_sets → workout_exercises → workout_sessions` join 同时绑定 owner、精确 workout 与精确 exercise，只选择 `id,position,kind,reps,display_load,display_load_unit,canonical_load_kg,duration_seconds,distance_meters,rpe,completed`；应用只保留末 set UUID，锚点子查询在同 owner/workout/exercise 内恢复 `position`，分页使用既有非部分唯一索引 `workout_sets_exercise_id_position_key`。真实数据库已证明软删除 workout 仍可读取组、其他 owner 排除、打开根快照后的当前与后续 exercise 并发 set 新增不可见，实际 set SQL 在禁用顺序扫描的计划验证中命中该索引。每层必须等待子流完整结束，最后边界后也必须显式 `complete()` 才提交；跳过、重复、提前停止和取消按最深活动子流优先回滚同一根事务。数据库实测还证明 API 合法的 30 个动作 × 50 组关系图在不含 history 时单项 JSON 已超过 64 KiB，且 `workout_revisions.snapshot` 保存完整聚合、修订数量没有上限。当前关系图已分层，下一步仍须先读取修订头，再递归分解 snapshot，不能把它作为单 payload。

递归 JSON 来源契约已经能表达 workout 对象内的懒 exercises/sets/history，并证明嵌套取消可传播到文件根；数据库现已把 workout 头、按 position 读取的动作和 set 标量放入同一根事务，并用分层收据分别记录三层批次/行数。下一项数据库工作应为单个 workout 按 revision 读取修订头，再评估如何递归分解每条完整 snapshot；不得截断历史或用空 history 形成不完整公开对象。
