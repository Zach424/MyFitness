...
训练同步投影按 `(started_at,created_at,id)` 升序输出顶层 workout；`UNIQUE (workout_id,position)`、`UNIQUE (exercise_id,position)` 与 `UNIQUE (workout_id,revision)` 分别保证动作、组和修订内部顺序。活动列表继续使用部分降序索引；迁移 0032 另建非部分 `(user_id,started_at,created_at,id)` 索引，服务包含软删除记录的便携导出全历史扫描。内部 `createWorkoutExerciseLayerSnapshot()` 在一次 active-owner、只读 repeatable-read 根事务中读取同一批 workout 头，并为每个头暴露一次性动作子流。动作查询通过 owner-bound workout join 限定父级，只选择 `id,position,exercise_key,name,category,notes,tracking_mode,equipment,equipment_notes`；应用只保留末动作 UUID，锚点子查询在同 owner/workout 内恢复 `position`，分页使用既有非部分唯一索引 `workout_exercises_workout_id_position_key`。真实数据库已证明软删除 workout 仍可读取动作、其他 owner 排除、打开根快照后的当前与后续 workout 并发动作新增不可见，实际动作 SQL 在禁用顺序扫描的计划验证中命中该索引。外层必须等待子流完整结束，最后边界后也必须显式 `complete()` 才提交；跳过、重复、提前停止和取消均回滚同一根事务。数据库实测还证明 API 合法的 30 个动作 × 50 组关系图在不含 history 时单项 JSON 已超过 64 KiB，且 `workout_revisions.snapshot` 保存完整聚合、修订数量没有上限。未来仍不能以 workout 为单 payload；sets 和修订还需继续在同一根事务中分层流式编码。

递归 JSON 来源契约已经能表达 workout 对象内的懒 exercises/sets/history，并证明嵌套取消可传播到文件根；数据库现已把 workout 头与按 position 读取的动作标量放入同一根事务，并用分层收据分别记录头与动作的批次/行数。下一项数据库工作应为单个 exercise 按 position 读取 set 标量，并让最深活动子流拥有优先清理责任；不得用空 history 形成不完整公开对象。
