...
训练同步投影按 `(started_at,created_at,id)` 升序输出顶层 workout；`UNIQUE (workout_id,position)`、`UNIQUE (exercise_id,position)` 与 `UNIQUE (workout_id,revision)` 分别保证动作、组和修订内部顺序。活动列表继续使用部分降序索引；迁移 0032 另建非部分 `(user_id,started_at,created_at,id)` 索引，服务包含软删除记录的便携导出全历史扫描。内部头部查询只选择 `id,title,status,source_kind,source_metadata,started_at,ended_at,timezone,pain_level,fatigue,note,revision,deleted_at,created_at,updated_at`，不携带 owner、幂等字段、请求哈希、动作或历史。真实数据库已证明反向插入后的三元组总序、其他 owner 排除、软删除保留、打开快照后的新增不可见；索引目录断言谓词为空，同一查询在禁用顺序扫描的计划验证中命中新索引。数据库实测还证明 API 合法的 30 个动作 × 50 组关系图在不含 history 时单项 JSON 已超过 64 KiB，且 `workout_revisions.snapshot` 保存完整聚合、修订数量没有上限。未来不能以 workout 为单 payload；还需把动作、组和修订在同一根事务中分层流式编码。

递归 JSON 来源契约已经能表达 workout 对象内的懒 exercises/sets/history，并证明嵌套取消可传播到文件根；数据库已提供独立会话头源，但尚未把它与嵌套来源放入同一根事务。下一项数据库工作应为单个 workout 按 position 读取动作标量，并让外层头游标在动作消费期间保持快照；不得用空 sets/history 形成不完整公开对象。
