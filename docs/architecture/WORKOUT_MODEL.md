...
`createWorkoutRevisionSnapshot()` 与多修订 `createWorkoutRevisionSnapshotLayerSnapshot()` 共用同一个单 revision 根→exercise→set 嵌套节点。shape 补充父级范围内 UUID 唯一；根、动作和修订信封分别由 PostgreSQL 交付保留 `exercises: []`、`sets: []`、`snapshot: null` 键位的 JSONB 骨架，Node 只原地替换现有值。页面用上一元素 UUID 恢复 JSON ordinality，因此合法反序 position 既不被重排，也不会因删除再追加键改变对象序。当前关系图必须先结束，每个修订 snapshot 又必须完整结束，才能推进下一 revision；真实 PostgreSQL 已证明完整 history 与同步 `{id,action,revision,snapshot,changed_at}` 逐字节相同。该内部结构尚未适配为公开完整 workout JSON。
...
The register retains one create idempotency key only while the definition draft is unchanged. A lost create response can be explicitly retried with that key and the server's owner/key/request-hash guard returns one definition. Correction and archive have no replay promise: the client first reloads the active owner catalog. An advanced revision is called a successful lost correction only when every submitted definition field matches; otherwise the page preserves the input against the newly read revision. Archive is called effective only after the active catalog no longer contains the entry, and that evidence means only “not available for future selection.” No action mutation is queued or replayed in the background. See [ADR-0052](decisions/0052-authority-aware-sensitive-workbench-recovery.md).
