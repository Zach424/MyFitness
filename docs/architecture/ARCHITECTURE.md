...
- 便携导出的 API 与客户端共享 50 MiB（52,428,800 字节）边界。API 先把 repeatable-read 结果格式化为最终 JSON 字符串，以 UTF-8 精确计数；超限时在分配附件 `Buffer` 前返回固定 413 `portable_export_too_large`，成功时显式发送同字节数的 `Content-Length`。隐私专用客户端适配器仍独立验证媒体类型、精确 v4 信封/集合拓扑、标识符、时间和同一边界，只有通过后 H5 才创建下载、WeApp 才持久保存，页面状态只接收版本、生成时间和字节长度。
