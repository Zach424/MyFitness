...
- 九个健康指标的规范单位与允许展示单位由无依赖 `metricUnitDefinitions` 统一定义；领域 `metricDefinitions` 在其上增加范围、整数与换算规则。健康来源构造器和共享响应 Schema 分别扫描全部原始点与公开点，拒绝已知但不属于精确指标的单位。该关系门禁不执行数值范围、不换算展示值，也不覆盖历史输入。
