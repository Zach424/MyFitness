...
- 异步归档使用共享 `myfitness-portable-export-archive-receipt/v1`、六状态单调转换和迁移 0029–0030。`privacy_export_archives` 以 owner/幂等 UUID/请求哈希固定意图，以确定性 `.json.enc` 键、非秘密密钥引用、SHA-256/安全整数大小和生成/下载期限固定保管事实；生命周期 CHECK 与更新触发器阻止跳级、回滚和同状态替换证据。内部 `PortableExportArchiveService` 在事务中锁定 active owner，服务端产生归档 UUID/对象键/一小时生成期限，依唯一约束把并发同请求收敛到同一收据，并拒绝同键不同哈希、跨 owner、非 active owner 和终态复活。收据不含对象键、密钥引用、URL 或访问令牌；`users` 使用 RESTRICT 外键。该服务没有公开控制器，本边界尚未提供执行器、加密对象写入、下载授权或 UI。
