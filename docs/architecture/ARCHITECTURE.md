...
- 单字段数值通过后，来源关系门禁验证 SQL 聚合本应保证的同一行关系：动作窗口 `session_count <= completed_set_count`，动作点 `0 < completed_set_count <= total_set_count`；健康窗口把记录数、记录日、统计存在性绑定，并要求 `minimum <= average <= maximum`。共享动作窗口/点和健康窗口 Schema 对转换后的响应重复验证相同关系；错误不被修复、排序或解释为趋势，健康负值仍合法。健康跨窗口关系已有独立门禁；动作跨窗口聚合单调性仍待验证。
