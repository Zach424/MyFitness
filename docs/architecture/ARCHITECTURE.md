...
状态：已接受并实现至第 099 轮产品简报中文权威迁移；架构变更必须新增 ADR。
...
- 中文项目记录由无第三方运行时依赖的 `myfitness-chinese-documentation/v2` 门禁约束。六份活跃权威文档都要求整体中文占比不低于 72%，且不得存在纯英文叙述行；产品简报额外锁定从“产品主张”到“商业模式假设”的 13 个导航标题。第 090 轮起的迭代档案和 ADR-0085 起的决策记录必须使用中文标题、元数据与以中文为主的正文。仓库状态仍是唯一权威来源，Obsidian 只保存逐字节镜像。
- 独立的 `myfitness-documentation-migration-index/v1` 递归扫描 `docs/` 下全部 Markdown，并把每份文件恰好归入活跃权威、治理、待迁移专题、待迁移历史或连续受保护编号序列。显式路径当前负责 18 份专题，六个闭区间负责迭代 000–089 与 ADR-0001–0084；第 090 轮及 ADR-0085 以后按连续编号自动受保护。检查器拒绝漏项、重复归类、登记缺失、编号缺口、陈旧活跃文档/待办总量、批次 ID 或专题链接，并在 CI 中紧随中文正文门禁执行；范围完整性不替代逐份语义和技术字面量审阅。
