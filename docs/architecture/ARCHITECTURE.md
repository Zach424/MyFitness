...
- 健康 90 日窗口和点查询共享所有者、精确指标、confirmed/current、参考时刻与 90 日资格边界。纯构造器在未来过滤与 181 行上界后绑定窗口 `recordCount`：不超过 180 时来源行数必须相等，超过 180 时必须收到恰好 181 行。公开 Schema 独立要求 `series.length = min(recordCount,180)` 且 `hasMore = (recordCount > 180)`；缺失 90 日来源行只在没有合格点时继续生成空窗口。该收据不从 181 行前缀臆测更多记录，也不重算最小值、最大值或平均值。
