...
- 异步归档使用共享 `myfitness-portable-export-archive-receipt/v1`、六状态单调转换和迁移 0029–0030。`privacy_export_archives` 固定意图、对象键、非秘密密钥引用、产物收据和期限；内部预约服务以 active owner 行锁和唯一约束收敛同请求并阻止跨 owner/终态复活。内部 `myfitness-portable-export-archive-envelope/v1` 进一步以 AES-256-GCM 分块认证字节流：54 字节有界头携带随机 nonce 前缀和 owner/archive/v4/key-ref 上下文摘要；每帧使用单调计数 nonce、完整头/帧 AAD 和 16 字节标签；认证空 final 帧加物理 EOF 拒绝帧边界截断和尾随。默认 64 KiB，解析只接受 1 KiB–1 MiB，完成收据只在完整消费后发布密文 SHA-256/大小。会话清零数据密钥副本，但生产 KMS lease、调用方密钥清零、JSON 流式快照、对象上传、执行器、下载授权和 UI 均未实现。
