...
- 当前采用计划和精确历史采用修订具有非持久化 `plan-outcome-review-v1`：从目标采用修订时刻开始观察七天，比较最近生成快照与采用快照的稳定选择，只连接同修订、采用后且仍活动的显式训练关联，以及采用后当前已确认的非 AI 恢复记录。历史路由按认证所有者、计划 UUID 与正整数 `accepted` 修订读取，统一隐藏外部/非采用目标并返回 `private, no-store`。解除关联、训练删除或恢复删除从证据中移除并以撤销计数说明；Unknown 与 Observed 仍只描述当前合格证据是否存在，不计算依从性、效果、因果或下一计划调整。客户端为单次历史读取维护独立加载/失败/重试权限和页面内存代次，不把初始失败发布为空结果，也不接受切换修订或卸载后的迟到响应。
