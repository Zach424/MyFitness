...
- 异步归档使用共享最小收据、六状态单调转换、迁移 0029–0035、只读 `REPEATABLE READ` 流事务、递归增量 JSON、AES-256-GCM 信封与有界 multipart writer。描述驱动协调器只校验一次 active owner，并按 v4 顺序交付前三个简单 keyset 源、两个条目/history 分层目录、完整七层 workouts 和完整 nutritionMeals。workout 与餐食会话都通过内部现有 `PoolClient` 适配器复用协调事务并跳过重复 owner 校验；各自 JSONB 键序、全部 keyset/ordinality、64 KiB 单元素门禁与最深层取消不变。餐食五段统计并入统一根收据，专用 JSON 适配器仍只原位挂载懒节点。根物理 EOF 才提交。后续集合/媒体、KMS lease、执行器、下载授权和 UI 仍未实现。
...
- JSON 来源类型以递归条件映射保留 `PrivacyExport` 根结构，并只接受私有 Symbol 标记的懒数组。七字段适配器递归包装两个目录 history、workout history/snapshot/current 关系与餐食 `items/history/snapshot.items`，并把同一协调数据库会话作为 JSON 根生命周期。`createPortableExportConsentHealthCatalogWorkoutNutritionJsonSource()` 组合既有 workout 与餐食数组适配器，不预取子流，也不创建第二完成权威；完整 EOF 完成协调根，任意深度早停取消同一根。真实数据库以 53 字节块证明完整七字段懒正文与同步 v4 逐字节相同，活动餐食历史 item 取消在 JSON、协调与数据库收据间保持同一错误。
