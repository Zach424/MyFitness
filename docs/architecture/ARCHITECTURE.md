...
状态：已接受并实现至第 108 轮个人状态证据账本；架构变更必须新增 ADR。
...
- 同一仪表板原子响应附带非持久化 `personal-state-ledger-v1`。账本只投影四个既有权威：同所有者且精确 `accepted` 修订的最新计划体验、恢复估计实际引用的已确认记录、七天趋势窗口和确定性恢复估计。严格契约把 `user_confirmed`、`confirmed`、`observed`、`estimated`/`unknown` 分开，并逐字段核对计数、最近证据、来源类型、状态、置信度和快照时间；每项 `validUntil` 固定为空，记录/时间/反思变化被声明为失效原因。仪表板返回 `private, no-store`，来源更正或删除后重新计算，不保存派生账本，也不产生 AI、因果、医疗或计划适应结论。
- 首页教练工作台是“今日”路由内的只读标签，复用同一次原子接受的仪表板和计划列表，不发起第二套读取。首段把本人确认、已确认恢复记录、七天系统观察和恢复估计/Unknown 并列，并公开快照无保证有效期；当前计划仍只选择包含本地日期的周范围，只统计与精确计划修订和课次日期同时匹配的显式训练关联。确认无计划、未读取和保留旧快照是不同状态；AI 档案入口在计划存在后才出现，且仍受计划页的同意、新鲜度和确定性安全校验约束。
...
确定性周计划规则、证据来源凭据、有界修订生命周期与限制记录在 [PLAN_MODEL.md](PLAN_MODEL.md)。ADR-0008 说明结构化规则路径为何先于模型编排；ADR-0047 将历史续页绑定到精确的所有者计划修订；ADR-0097 要求主观恢复估计先通过个人基线、覆盖、方向一致性和置信度门槛，才可改变计划边界；ADR-0098 把状态证据、采用调整和后续确认记录连成修订绑定的非因果回看；ADR-0099 使历史采用修订按所有者与精确版本实时重算，并把撤销证据与 Unknown 分开；ADR-0103 以非持久化证据账本并列本人确认、确认记录、系统观察和估计/Unknown，同时保持既有权威与失效边界。
