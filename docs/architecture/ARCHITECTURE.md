...
- 异步归档先落地共享 `myfitness-portable-export-archive-receipt/v1` 最小收据、六状态单调转换和迁移 0029，而不暴露未执行的用户 API。`privacy_export_archives` 以 owner/幂等 UUID/请求哈希固定意图，以确定性 `.json.enc` 键、非秘密密钥引用、SHA-256/大小和生成/下载期限固定保管事实；生命周期 CHECK 与更新触发器阻止跳级、回滚和同状态替换证据。收据不含对象键、密钥引用、URL 或访问令牌。`users` 使用 RESTRICT 外键，保证归档对象处置并删除保管行前账号不能物理删除。本边界尚未提供执行器、对象写入、下载授权或 UI。
