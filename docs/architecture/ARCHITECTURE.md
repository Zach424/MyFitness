...
- 异步归档使用共享 `myfitness-portable-export-archive-receipt/v1`、六状态单调转换和迁移 0029–0030。`privacy_export_archives` 固定意图、对象键、非秘密密钥引用、产物收据和期限；内部预约服务以 active owner 行锁和唯一约束收敛同请求并阻止跨 owner/终态复活。精确 v4 JSON 字节源按既有两空格缩进和末尾换行增量编码所有属性名、标量、数组、对象、Unicode 与孤立代理项；配置范围为 1 字节至 1 MiB、默认 64 KiB，完整消费后才发布明文字节数/SHA-256，超限与取消都拒绝。它不分配完整格式化字符串或明文附件 `Buffer`，并已在超过 8 MiB 的测试中直接喂给 `myfitness-portable-export-archive-envelope/v1`。信封以 AES-256-GCM 分块认证字节流：54 字节有界头携带随机 nonce 前缀和 owner/archive/v4/key-ref 上下文摘要；每帧使用单调计数 nonce、完整头/帧 AAD 和 16 字节标签；认证空 final 帧加物理 EOF 拒绝帧边界截断和尾随。信封解析只接受 1 KiB–1 MiB，完成收据只在完整消费后发布密文 SHA-256/大小。会话清零数据密钥副本，但当前 JSON 源仍接收完整 `PrivacyExport`；生产数据库游标、KMS lease、调用方密钥清零、对象上传、执行器、下载授权和 UI 均未实现。
