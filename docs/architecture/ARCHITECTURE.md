...
- 只读洞察投影查询已确认/当前来源行，不持久化重复状态。仪表板生成“今日”证据、`subjective-recovery-state-v1` 主观恢复估计和跨领域总计。恢复估计按本地日期和指标去重，用 7 天近期窗口对比此前 28 天个人基线，并公开记录 UUID、修订、发生时间、来源、覆盖、方向一致性、置信度与限制；覆盖不足保持 Unknown，缺少基线或方向冲突保持低置信，只有中等置信且方向一致的估计可进入确定性计划证据。Dashboard 趋势、动作洞察与健康洞察的三个窗口共享精确 `[7, 30, 90]` 身份顺序，重复、缺失替代或错序会在对应 `days` 路径失败。动作按一个稳定键/已完成组进行分组；饮食只接受截至参考时刻本地日的连续 90 日，以空值表示缺失证据，并把固定顺序的 7/30/90 日计数、覆盖和营养总计逐字段绑定到该序列；错序、重复、未来、缺日或偏移失败关闭，不自动补零。健康在规范单位中按一个精确已确认指标分组，同时保留展示/来源/时区/修订凭据。健康/动作查询已由 SQL 使用 `<= at` 并按发生时刻、创建时刻和 UUID 降序；公开纯构造器在未来过滤、180 点截断、首点身份/单位和 `hasMore` 推导前验证响应时区及全部输入行的 `occurred_at` 非递增，点的 `localDate` 直接由 `occurredAt + response timezone` 推导。共享 Schema 独立拒绝无效响应时区、逐点日期错位、公开序列升序断点及 `occurredAt > generatedAt`；相同发生时刻允许，因为公开响应不含 `createdAt`，健康记录自身的 `recordTimezone` 也不替代响应时区。窗口聚合没有逐点时间，仍由数据库查询资格保证，不从有限点序列反推。来源更正/删除后，每个投影都会重新计算。
