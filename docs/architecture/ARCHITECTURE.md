...
- 客户端把食物照片与进度照片注册为两个独立 Taro 延迟页面，避免任一用途的实现静态进入另一用途的页面块。H5 只把至少被两个异步页面共同使用的运行时、共享组件和工具抽入 `client-async-shared`；发生时间字段保留高优先级专用块，应用入口只加载轻量时区探测器。该拆包不合并照片同意、保留、API 或读取权限，WeApp 仍由页面边界独立产出。
