...
- 健康点来源先以原始 `source_kind + source_metadata` 通过共享 `recordSourceSchema`，防止非对象容器在公开映射中被折叠为空；随后完整映射点通过 `healthInsightPointSchema`。该边界验证单位枚举、严格有界元数据、有效 IANA 记录时区，并拒绝 `ai_estimate` 进入 confirmed-only 洞察。全部原始点在顺序、未来过滤与截断前受检；记录时区不替代响应时区，负值不据此解释为医疗异常。
