...
- 同一仪表板原子响应附带非持久化 `personal-state-ledger-v1`。账本只投影四个既有权威：同所有者且精确 `accepted` 修订的最新计划体验、恢复估计实际引用的已确认记录、七天趋势窗口和确定性恢复估计。Dashboard 的健康/餐食 `occurred_at`、训练 `started_at` 与计划体验 `updated_at` 共用不晚于参考时刻的资格边界；参数化 SQL、纯投影和共享 Schema 分别阻断未来事实。该 `at` 只表示发生时间截止，不按 `created_at` 或修订 `changed_at` 重建事务时间历史。严格契约把 `user_confirmed`、`confirmed`、`observed`、`estimated`/`unknown` 分开，并逐字段核对计数、最近证据、来源类型、状态、置信度和快照时间；每项 `validUntil` 固定为空，记录/时间/反思变化被声明为失效原因。仪表板返回 `private, no-store`，来源更正或删除后重新计算，不保存派生账本，也不产生 AI、因果、医疗或计划适应结论。
