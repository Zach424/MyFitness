...
- 异步归档使用共享 `myfitness-portable-export-archive-receipt/v1`、六状态单调转换和迁移 0029–0030。`privacy_export_archives` 固定意图、对象键、非秘密密钥引用、产物收据和期限；内部预约服务以 active owner 行锁和唯一约束收敛同请求并阻止跨 owner/终态复活。数据库层提供延迟开启的只读 `REPEATABLE READ` 流事务：只有源到达物理 EOF 才提交，查询错误、AbortSignal 或调用方提前停止都会回滚并释放连接，原错与回滚错以有序聚合保留。首个健康记录源按默认 25、最大 100 行 keyset 页面读取，只保存末 UUID，并在相同 PostgreSQL 快照内用锚点子查询比较 `(occurred_at, created_at, id)`，避免 JavaScript `Date` 丢失微秒。精确 v4 JSON 字节源允许各数组字段使用私有 Symbol 标记的同步/异步 iterable；常规数组与懒数组共享同一个递归 token、两空格缩进、Unicode 转义、固定 UTF-8 块、上限和 SHA-256 逻辑。只有遍历到字段时才请求元素，源错误或取消会沿异步迭代器关闭链传播；完整消费后才发布明文字节数/SHA-256。真实 PostgreSQL 已验证健康记录行源嵌入字段完整 v4 后与 eager 输出逐字节相同，并在中途停止时回滚快照。字节源直接进入 `myfitness-portable-export-archive-envelope/v1` 的 AES-256-GCM 认证帧。私有对象 writer 再把未知总长密文顺序聚合为默认 8 MiB、允许 5–64 MiB 的 S3 multipart 部件，最多 10,000 个；每部件提交 SHA-256，最终 `If-None-Match: *` 条件完成后才返回完整密文字节数/SHA-256。读取背压来自串行 `UploadPart`；完成前的源错误、AbortSignal、部件失败与 412 都使用独立清理请求中止，abort 失败以双错误上报。真实 MinIO 已验证超过 5 MiB 的 JSON → 信封 → 两部件对象、原对象冲突保护及两种中途失败零残留。最终完成响应丢失仍可能使对象成功但调用方未知，必须由未来执行器按对象键/摘要对账。现有 Buffer 上传保持不变，SSE 配置由两条路径共享。当前只有健康记录数组完成数据库→v4 懒组合，其余集合、嵌套聚合和媒体仍完整驻留，multipart writer 也尚未连接归档数据库状态；KMS lease、调用方密钥清零、执行器、下载授权和 UI 均未实现。
