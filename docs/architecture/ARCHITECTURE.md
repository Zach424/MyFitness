...
- JSON 来源类型以递归条件映射保留 `PrivacyExport` 根结构，并只接受私有 Symbol 标记的懒数组。六字段适配器递归包装两个目录 history，以及 workout history、snapshot exercises/sets 和当前 exercises/sets，并把同一协调数据库会话作为 JSON 根生命周期。独立餐食会话让 PostgreSQL 生成 `items: []`、`history: []`、`snapshot: null` 三类骨架，Node 只在同步 JSONB 的 `items→history` 顺序原位挂载当前 items、revision 和 snapshot items 子流。真实数据库证明完整物化餐食与同步 v4 逐字节相同；最深叶超限或取消都在根事务失败关闭。下一结构依赖是递归餐食 JSON 适配器，随后才把现有会话注入第七协调字段。
