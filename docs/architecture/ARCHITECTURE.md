...
- JSON 来源类型以递归条件映射保留 `PrivacyExport` 根结构，并只接受私有 Symbol 标记的懒数组。六字段适配器递归包装两个目录 history，以及 workout history、snapshot exercises/sets 和当前 exercises/sets，并把同一协调数据库会话作为 JSON 根生命周期。适配单元证明完整 workout 键位；真实数据库以 47 字节块证明六字段 eager/lazy 等价。下一结构依赖是审计并分层 `nutritionMeals` 的当前 items、history 与 revision snapshot items。
