...
- 九个健康指标的规范单位与允许展示单位由无依赖 `metricUnitDefinitions` 统一定义；九个单位到规范单位的换算因子、四位持久化小数精度和一致性判断同样是 contracts/domain 共用纯规则。领域 `metricDefinitions` 在其上增加范围与整数规则，并复用换算函数保持既有四位规范值写入。健康来源构造器和共享响应 Schema 分别扫描全部原始点与公开点：先拒绝不属于精确指标的单位，再按 `NUMERIC(14,4)` 的规范值半量化误差加换算后展示值半量化误差判断两个值是否代表同一测量；相同单位不重复加入展示舍入误差。检查覆盖未来点与隐藏第 181 点，错误值失败关闭而不自动修复、覆盖历史输入或执行健康范围判断。
