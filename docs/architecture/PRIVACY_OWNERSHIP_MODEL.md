...
- 完整数据库行、非媒体 JavaScript 对象图和格式化下界字符串仍在 API 内存中生成；正常请求还会生成含真实媒体的最终字符串。服务端已能在媒体读取前拒绝“下界本身超限”的请求，并在最终 UTF-8 超出共享 50 MiB 时避免附件 `Buffer`，但这不是流式或异步加密归档；小程序真机下载、读取和保存行为也尚未演练。
