...
异步归档的数据库保管模型与同步下载保持分离。`privacy_export_archives` 的六状态、RESTRICT owner 外键、触发器和内部预约服务固定意图、单调状态与所有者读取；确定性对象键和 key-ref 只留在服务端。数据库流事务以只读 `REPEATABLE READ` 固定一个所有者快照，只有内部源到达物理 EOF 才提交；提前停止、查询错误或主动取消都会回滚并释放连接。`createHealthHistorySnapshot()` 只校验一次 active owner，再按 v4 顺序依次读取 `healthRecords` 与 `healthRecordRevisions`；两者分别按 25/最大 100 行读取，续页只保留不含健康内容的末 UUID，并在同一快照中解析精确排序元组。数据库先把单行投影编码成 JSON 文本并精确计量 UTF-8 字节，只让不超过 64 KiB 的 payload 跨入 Node；超限错误只携带上限和实际字节数，不暴露记录 ID 或内容。内部 v4 JSON 字节源用私有 Symbol 节点放置两个异步行源，根生命周期在双字段结束后仍保管暂停事务：只有整个 JSON 物理 EOF 才 `complete()` 并提交，活动字段、字段间空隙或后续字段取消均 `cancel()` 并以同一根错误拒绝统一分集合收据。真实 PostgreSQL 已证明第一字段结束后的并发写入不会进入第二字段、双懒数组与 eager v4 逐字节一致，以及字段间取消时修订源尚未启动。该字节源直接进入分块 AES-256-GCM codec，后者认证格式、owner、archive、v4、key-ref 与完整 EOF。密文再由 multipart writer 按 5–64 MiB 有界部件串行写入，每部件 SHA-256 由存储方验证；只有最终条件创建成功才发布完整存储字节数/SHA-256。任何源错误、主动取消、部件失败或既有对象竞争都会中止未完成 upload；中止本身失败时同时保留原错与清理错，不能发布完成。其他集合/嵌套聚合/媒体仍完整驻留，writer 也尚未连接归档状态。没有 KMS 数据密钥 lease、公开预约路由、生成任务、签名下载端点或客户端入口，因此这仍不是可用功能声明。
...
- 异步归档目前已有共享状态/最小收据、PostgreSQL 保管约束、所有者事务预约、只读流事务、健康记录与修订有界 keyset 行源、同事务多集合协调器、64 KiB 单 payload 交付门禁、根生命周期懒数组 JSON 编码、内部认证加密 codec 和有界 multipart 对象写入；两个集合已经完成同一快照、根 EOF 提交、字段间取消回滚和字段完整 v4 的组合证明，但其他集合、嵌套聚合和媒体仍完整驻留，而且这些组件尚无受租约执行器统一状态。该门禁限制数据库向 Node 交付的单元素文本，不限制 PostgreSQL 内部编码成本，也不替代完整文件 50 MiB 门禁。没有 KMS lease、短期下载授权、保留扫描、账号擦除协调或用户入口，不能把 `available` 状态用于任何真实请求。
