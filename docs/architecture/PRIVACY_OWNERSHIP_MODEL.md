...
异步归档的数据库保管模型与同步下载保持分离。`privacy_export_archives` 的状态为 `queued/generating/available/failed/deletion_pending/disposed`；只有 `generating → available` 可以首次发布摘要、安全整数大小与可下载期限，只有 `deletion_pending → disposed` 可以清除对象键和密钥引用，失败状态不持有对象。确定性 `.json.enc` 键和加密密钥引用只留在数据库，最小收据只含归档 UUID、状态、时刻、摘要/大小与受控失败/处置。内部预约服务在事务中锁定 active owner，由服务端生成归档 UUID、对象键和一小时生成期限；owner/幂等 UUID 唯一约束让并发同请求返回同一收据，同键不同哈希冲突，owner+archive 读取、非 active 账户和 failed/disposed 重放均失败关闭或保持终态。数据库 RESTRICT 所有者删除并以触发器拒绝跳级、回滚、时间倒退与同状态保管字段变更。当前没有公开预约路由、生成任务、加密归档对象、签名下载端点或客户端入口，因此该内部服务仍不是可用功能声明。
...
- 异步归档目前已有共享状态/最小收据、PostgreSQL 保管约束和不公开的所有者事务预约服务；没有生成执行器、加密对象写入、短期下载授权、保留扫描、账号擦除协调或用户入口，不能降低同步导出压力，也不能把 `available` 状态用于任何真实请求。
