...
- 异步归档目前已有共享状态/预约、所有者只读流事务、同意/健康/修订/两个目录/完整 workouts 六字段协调来源、64 KiB 门禁、根生命周期 JSON、认证加密和 multipart 写入。两个目录都含活动与归档 owner 条目而不含 starter；workout 按 JSONB `history→exercises` 在同一事实时刻读取。`nutritionMeals`、后续集合/媒体、KMS lease、状态执行、限时下载、保留扫描、账号擦除协调和用户入口仍缺失。
