...
- 异步归档目前已有共享状态/最小收据、PostgreSQL 保管约束、所有者事务预约、只读流事务、同意事件/健康记录/健康修订有界 keyset 行源、描述驱动同事务协调器、64 KiB 单 payload 交付门禁、根生命周期懒数组 JSON 编码、内部认证加密 codec 和有界 multipart 对象写入；三个集合已经完成同一快照、根 EOF 提交、字段间取消回滚和字段完整 v4 的组合证明，但其他十个顶层集合、嵌套聚合和媒体仍完整驻留，而且这些组件尚无受租约执行器统一状态。该门禁限制数据库向 Node 交付的单元素文本，不限制 PostgreSQL 内部编码成本，也不替代完整文件 50 MiB 门禁。没有 KMS lease、短期下载授权、保留扫描、账号擦除协调或用户入口，不能把 `available` 状态用于任何真实请求。
- 训练所有权导出包含当前关系图、软删除会话和完整修订历史。同步投影已用 UUID 补全同时间顶层顺序，父级唯一 position/revision 保证嵌套顺序；但一个现有契约允许的 30×50 当前关系图在不含历史时已经超过 64 KiB。把它作为一行会拒绝合法用户证据，提高门禁则会恢复无界 Node 分配。后续必须在一个 owner 快照中分层读取训练头、动作、组和修订，并证明任何内层取消都回滚根事务；活动列表部分索引不能替代包含软删除历史的导出索引。
- 内部 JSON 来源类型现在允许 workouts 内的 exercises、sets 和 history 使用显式私有懒节点。嵌套测试证明内层数据在字段到达前不读取，首个 set 后停止会先执行其来源清理，再以同一错误取消根生命周期；因此未来数据库事务可以接入这一关闭链。该测试没有数据库 owner 查询、软删除历史或分层收据，不能作为训练导出完成证据。
