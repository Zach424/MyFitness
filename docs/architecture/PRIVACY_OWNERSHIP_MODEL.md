...
异步归档的数据库保管模型与同步下载保持分离。`privacy_export_archives` 的六状态、RESTRICT owner 外键、触发器和内部预约服务固定意图、单调状态与所有者读取；确定性对象键和 key-ref 只留在服务端。内部 v4 JSON 字节源不拼接完整格式化字符串或附件 `Buffer`，完整消费后才提供明文字节数/SHA-256。该字节源直接进入分块 AES-256-GCM codec，后者认证格式、owner、archive、v4、key-ref 与完整 EOF。密文再由 multipart writer 按 5–64 MiB 有界部件串行写入，每部件 SHA-256 由存储方验证；只有最终条件创建成功才发布完整存储字节数/SHA-256。任何源错误、主动取消、部件失败或既有对象竞争都会中止未完成 upload；中止本身失败时同时保留原错与清理错，不能发布完成。真实 MinIO 组合测试已证明两部件保管与三个失败路径无覆盖、无可见错误对象、无 multipart 残留。三类收据都是未来执行器内部证据，不进入公共最小收据。当前 JSON 字节源仍消费完整 `PrivacyExport`，writer 尚未连接归档状态，且没有数据库游标、KMS 数据密钥 lease、公开预约路由、生成任务、签名下载端点或客户端入口，因此这仍不是可用功能声明。
...
- 异步归档目前已有共享状态/最小收据、PostgreSQL 保管约束、所有者事务预约、增量 JSON 字节编码、内部认证加密 codec 和有界 multipart 对象写入；但字节编码的输入仍是完整 `PrivacyExport`，且这些组件尚无受租约执行器统一状态。没有有界数据库快照读取、KMS lease、短期下载授权、保留扫描、账号擦除协调或用户入口，不能把 `available` 状态用于任何真实请求。
