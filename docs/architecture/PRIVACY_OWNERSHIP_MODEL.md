...
- 异步归档目前已有共享状态/最小收据、PostgreSQL 保管约束、所有者事务预约、只读流事务、同意事件/健康记录/健康修订有界 keyset 行源、独立 workout 会话头源、描述驱动三集合协调器、64 KiB 单 payload 交付门禁、根生命周期递归懒数组 JSON 编码、内部认证加密 codec 和有界 multipart 对象写入。三个已协调集合完成同一快照、根 EOF 提交、字段间取消回滚和字段完整 v4 的组合证明；workout 头源另完成 owner/软删除/并发隔离、非部分索引计划与取消证明，但没有进入协调器或公开 v4。其他十个顶层集合、嵌套聚合和媒体仍完整驻留，而且这些组件尚无受租约执行器统一状态。该门禁限制数据库向 Node 交付的单元素文本，不限制 PostgreSQL 内部编码成本，也不替代完整文件 50 MiB 门禁。没有 KMS lease、短期下载授权、保留扫描、账号擦除协调或用户入口，不能把 `available` 状态用于任何真实请求。
- 训练所有权导出包含当前关系图、软删除会话和完整修订历史。同步投影已用 UUID 补全同时间顶层顺序，父级唯一 position/revision 保证嵌套顺序；但一个现有契约允许的 30×50 当前关系图在不含历史时已经超过 64 KiB。把它作为一行会拒绝合法用户证据，提高门禁则会恢复无界 Node 分配。内部头源现在用 active owner 校验、全历史 `(started_at,created_at,id)` keyset 和非部分索引读取 15 个会话标量，软删除仍是用户证据，owner/幂等/请求哈希不进入 payload；真实快照隔离和取消收据已通过。后续仍必须在同一 owner 根事务中分层读取动作、组和修订，并证明任何内层取消都回滚根事务。
- 内部 JSON 来源类型现在允许 workouts 内的 exercises、sets 和 history 使用显式私有懒节点。嵌套测试证明内层数据在字段到达前不读取，首个 set 后停止会先执行其来源清理，再以同一错误取消根生命周期。独立 workout 头源已经补上数据库 owner 查询和软删除会话证据，但仍没有动作/组/修订嵌套来源或分层收据，不能作为训练导出完成证据。
