...
- 异步归档目前已有共享状态/预约、所有者只读流事务、同意/健康/修订三字段协调来源、完整 workout 懒 JSON 独立来源、64 KiB 单元素门禁、根生命周期 JSON 编码、认证加密和 multipart 写入。workout JSON 模式只校验一次 active owner，并按 JSONB 的 `history→exercises` 键序在同一事实时刻读取完整修订证据与当前关系；数据库占位键原位替换，未知 shape 在正文前关闭，取消优先清理最深活动 set。它尚未与前三字段共享事务，且 v4 顺序中的 exercise/food catalog、后续集合和媒体仍未有界完成。没有 KMS lease、状态执行、限时下载、保留扫描、账号擦除协调或用户入口，不能把内部来源当成可用归档。
