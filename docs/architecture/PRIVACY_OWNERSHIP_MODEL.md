...
- 单 revision 的 `inspectWorkoutRevisionSnapshotShape()` 只返回兼容分类、计数、字节数、顺序布尔与 `decomposable`，不泄露账号、workout/revision UUID 或训练正文。它沿父链复核 owner 和 snapshot 身份，以严格允许键拒绝静默丢字段，并在 PostgreSQL 内证明根头、动作头和 set 各自不超过 64 KiB。历史数组的 JSON ordinality 属于不可变证据：合法反序 position 只令顺序标志为 false，不能成为重排或拒绝理由。该收据只证明可安全拆分，不是今天完整 Workout 领域规则的重新验证。
- 内部 JSON 来源类型现在允许 workouts 内的 exercises、sets 和 history 使用显式私有懒节点。嵌套测试证明内层数据在字段到达前不读取，首个 set 后停止会先执行其来源清理，再以同一错误取消根生命周期。数据库已有 workout→exercise→set→revision header 同根查询、四段收据和 snapshot shape 前置门禁，但 history 元素仍只有头部，缺少按 JSON ordinality 的完整 snapshot 递归来源与完整 v4 组合，不能作为训练导出完成证据。
