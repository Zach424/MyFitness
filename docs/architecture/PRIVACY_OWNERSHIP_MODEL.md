...
- 异步归档目前已有共享状态/最小收据、PostgreSQL 保管约束、所有者事务预约、只读流事务、同意事件/健康记录/健康修订有界 keyset 行源、训练当前图与完整 revision snapshot 七层同根来源、描述驱动三集合协调器、64 KiB 单 payload 交付门禁、根生命周期递归懒数组 JSON 编码、内部认证加密 codec 和有界 multipart 对象写入。训练组合只校验一次 active owner，关系图和逐 revision 快照共享同一事实时刻，未知 shape 在修订正文前关闭，任意取消优先清理最深活动 set。它尚未适配成完整 workout JSON 或进入跨顶层协调/公开 v4；其他十个顶层集合和媒体仍完整驻留，而且这些组件尚无受租约执行器统一状态。该门禁限制数据库向 Node 交付的单元素文本，不限制 PostgreSQL 内部编码成本，也不替代完整文件 50 MiB 门禁。
...
- 内部 JSON 来源类型允许 workouts 内的 exercises、sets、history 和 revision snapshot 子数组使用显式私有懒节点。数据库 `createWorkoutRevisionSnapshotLayerSnapshot()` 已把当前关系图与完整不可变 history 组合成一个 owner-scoped 根事务，并以七层收据、逐修订门禁和最深层取消证明责任边界；但其 `{header,exercises,history}` 仍是数据库内部结构，尚未转换成编码器可直接消费的完整 workout 值，也没有进入协调器或公开 v4。
