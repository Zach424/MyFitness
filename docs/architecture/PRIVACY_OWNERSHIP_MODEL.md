...
异步归档的数据库保管模型与同步下载保持分离。`privacy_export_archives` 的六状态、RESTRICT owner 外键、触发器和内部预约服务固定意图、单调状态与所有者读取；确定性对象键和 key-ref 只留在服务端。内部 v4 JSON 字节源不拼接完整格式化字符串或附件 `Buffer`，逐段保持既有字段顺序、两空格缩进、字符串转义、Unicode 和末尾换行；完整消费后才提供明文字节数/SHA-256。该字节源可直接进入分块 AES-256-GCM codec，后者把格式、owner、archive、v4 与非秘密 key-ref 摘要纳入每帧 AAD；随机前缀加帧序号形成唯一 nonce，认证空 final 帧和物理 EOF 拒绝截断/尾随。解析器在读取帧长度前先限制头部声明块，单帧认证后才输出明文，完成收据只在整个输出消费完毕后提供密文 SHA-256/大小。两类收据都是未来执行器内部证据，不进入公共最小收据。当前 JSON 字节源仍消费完整 `PrivacyExport`，且没有数据库游标、KMS 数据密钥 lease、公开预约路由、生成任务、归档对象、签名下载端点或客户端入口，因此这仍不是可用功能声明。
...
- 异步归档目前已有共享状态/最小收据、PostgreSQL 保管约束、所有者事务预约、增量 JSON 字节编码和内部认证加密 codec；但字节编码的输入仍是完整 `PrivacyExport`。没有有界数据库快照读取、KMS lease、生成执行器、流式对象写入、短期下载授权、保留扫描、账号擦除协调或用户入口，不能把 `available` 状态用于任何真实请求。
