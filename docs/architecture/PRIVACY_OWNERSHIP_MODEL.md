...
- 异步归档目前已有共享状态/预约、所有者只读流事务、同意/健康/修订/两个目录/完整 workouts 六字段协调来源、64 KiB 门禁、根生命周期 JSON、认证加密和 multipart 写入。两个目录都含活动与归档 owner 条目而不含 starter；workout 按 JSONB `history→exercises` 在同一事实时刻读取。餐食已有 owner-safe 无正文 shape 收据、UUID 顶层总序和非部分索引；正文分层、第七字段、后续集合/媒体、KMS lease、状态执行、限时下载、保留扫描、账号擦除协调和用户入口仍缺失。
- 餐食所有权导出必须包含软删除 meal、当前 items 与全部完整修订快照。`inspectNutritionMealShape()` 在同一只读事实边界内用计数与字节数证明 30-item、4-revision 合法历史已经超过 64 KiB，但不会让 owner/meal UUID 或餐食正文跨出数据库。该收据不是正文导出，也不能替代逐层 64 KiB 门禁；下一实现必须按 meal、当前 items、history 头和 revision snapshot items 分层，并在任何内层提前停止时回滚根事务。
