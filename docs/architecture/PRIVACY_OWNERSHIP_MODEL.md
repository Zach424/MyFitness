...
异步归档的数据库保管模型与同步下载保持分离。`privacy_export_archives` 的六状态、RESTRICT owner 外键、触发器和内部预约服务固定意图、单调状态与所有者读取；确定性对象键和 key-ref 只留在服务端。数据库流事务以只读 `REPEATABLE READ` 固定一个所有者快照，只有内部源到达物理 EOF 才提交；提前停止、查询错误或主动取消都会回滚并释放连接。`healthRecords` 与 `healthRecordRevisions` 行源分别按 25/最大 100 行读取，续页只保留不含健康内容的末 UUID，并在各自快照中解析精确排序元组；非 active owner 和跨 owner 行不会进入源。数据库先把单行投影编码成 JSON 文本并精确计量 UTF-8 字节，只让不超过 64 KiB 的 payload 跨入 Node；超限错误只携带上限和实际字节数，不暴露记录 ID 或内容。内部 v4 JSON 字节源可用私有 Symbol 节点把任一异步行源放入对应数组，只有编码到该字段才取下一行，不创建该集合数组；它仍保持既有字段顺序、转义、缩进和末尾换行，完整消费后才提供明文字节数/SHA-256。源错误、超限与活动数组中途停止会关闭嵌套迭代器，使数据库和 JSON 收据以同一根错误共同失败。该字节源直接进入分块 AES-256-GCM codec，后者认证格式、owner、archive、v4、key-ref 与完整 EOF。密文再由 multipart writer 按 5–64 MiB 有界部件串行写入，每部件 SHA-256 由存储方验证；只有最终条件创建成功才发布完整存储字节数/SHA-256。任何源错误、主动取消、部件失败或既有对象竞争都会中止未完成 upload；中止本身失败时同时保留原错与清理错，不能发布完成。真实 PostgreSQL/MinIO 组合测试分别证明两个独立 lazy v4 行源、单行超限隔离与两部件保管。两个数据库源尚未共享事务和根生命周期，因此不能同时用于一份一致 v4；其他集合/嵌套聚合/媒体仍完整驻留，writer 也尚未连接归档状态。没有 KMS 数据密钥 lease、公开预约路由、生成任务、签名下载端点或客户端入口，因此这仍不是可用功能声明。
...
- 异步归档目前已有共享状态/最小收据、PostgreSQL 保管约束、所有者事务预约、只读流事务、健康记录与修订两个独立有界 keyset 行源、64 KiB 单 payload 交付门禁、可组合懒数组 JSON 编码、内部认证加密 codec 和有界 multipart 对象写入；两个集合各自完成数据库→字段完整 v4 的惰性组合与超限失败证明，但尚未共享同一事务/根取消生命周期，其他集合、嵌套聚合和媒体也仍完整驻留，而且这些组件尚无受租约执行器统一状态。该门禁限制数据库向 Node 交付的单元素文本，不限制 PostgreSQL 内部编码成本，也不替代完整文件 50 MiB 门禁。没有 KMS lease、短期下载授权、保留扫描、账号擦除协调或用户入口，不能把 `available` 状态用于任何真实请求。
