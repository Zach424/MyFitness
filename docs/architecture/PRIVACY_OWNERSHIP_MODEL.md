...
异步归档的数据库保管模型与同步下载保持分离。数据库流事务以只读 `REPEATABLE READ` 固定一个 active owner 快照，只有 JSON 物理 EOF 才提交；提前停止、查询错误或主动取消都会回滚。当前协调入口按 v4 顺序读取同意事件、健康记录、健康修订和 owner 自定义动作目录，前三者为简单 keyset 行源，目录条目则以 `history: []` 骨架嵌套 revision 子流。内置 starter 不属于用户数据；活动与归档自定义条目都属于导出/擦除范围。所有 payload 在 PostgreSQL 内执行 64 KiB UTF-8 门禁，续页只保留末 UUID。嵌套失败会先以具体 history 错误关闭根事务，四字段 JSON/数据库收据共享同一失败。真实 PostgreSQL 已证明跨字段并发隔离、跨 owner/starter 排除和 eager/lazy 字节一致。认证加密与 multipart writer 已独立验证，但 `foodCatalog`、workouts 同根连接、后续集合/媒体、KMS、状态执行、签名下载和 UI 仍缺失，因此这不是可用归档声明。
...
- 异步归档目前已有共享状态/预约、所有者只读流事务、同意/健康/修订/动作目录四字段协调来源、完整 workout 懒 JSON 独立来源、64 KiB 门禁、根生命周期 JSON、认证加密和 multipart 写入。动作目录含活动与归档 owner 条目，不含 starter；条目/revision 分层避免无上限 history 聚合。workout 仍按 JSONB `history→exercises` 在独立事实时刻读取。`foodCatalog`、workouts 同根协调、后续集合/媒体、KMS lease、状态执行、限时下载、保留扫描、账号擦除协调和用户入口仍缺失。
