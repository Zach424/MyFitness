...
- 异步归档目前已有共享状态/最小收据、PostgreSQL 保管约束、所有者事务预约、只读流事务、同意事件/健康记录/健康修订有界 keyset 行源、workout→exercise→set→revision header 同根分层源、描述驱动三集合协调器、64 KiB 单 payload 交付门禁、根生命周期递归懒数组 JSON 编码、内部认证加密 codec 和有界 multipart 对象写入。三个已协调集合完成同一快照、根 EOF 提交、字段间取消回滚和字段完整 v4 的组合证明；训练分层源另完成 owner/软删除/关系与修订总序/并发隔离、既有索引计划、显式根提交与最深活动子流优先清理证明，但修订 snapshot 尚未分解，也没有进入协调器或公开 v4。其他十个顶层集合和媒体仍完整驻留，而且这些组件尚无受租约执行器统一状态。该门禁限制数据库向 Node 交付的单元素文本，不限制 PostgreSQL 内部编码成本，也不替代完整文件 50 MiB 门禁。没有 KMS lease、短期下载授权、保留扫描、账号擦除协调或用户入口，不能把 `available` 状态用于任何真实请求。
...
- 内部 JSON 来源类型现在允许 workouts 内的 exercises、sets 和 history 使用显式私有懒节点。数据库已有 workout→exercise→set→revision header 同根查询和单 revision snapshot→exercise→set 原序正文，但两者仍是独立会话；history 元素缺少完整 snapshot 连接与完整 v4 组合，不能作为训练导出完成证据。
