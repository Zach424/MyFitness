...
- 异步归档目前已有共享状态/最小收据、PostgreSQL 保管约束、所有者事务预约、只读流事务、同意事件/健康记录/健康修订有界 keyset 行源、workout→exercise→set 同根分层源、描述驱动三集合协调器、64 KiB 单 payload 交付门禁、根生命周期递归懒数组 JSON 编码、内部认证加密 codec 和有界 multipart 对象写入。三个已协调集合完成同一快照、根 EOF 提交、字段间取消回滚和字段完整 v4 的组合证明；训练分层源另完成 owner/软删除/动作与组总序/并发隔离、既有位置索引计划、显式根提交与最深活动子流优先清理证明，但没有进入协调器或公开 v4。其他十个顶层集合、训练修订和媒体仍完整驻留，而且这些组件尚无受租约执行器统一状态。该门禁限制数据库向 Node 交付的单元素文本，不限制 PostgreSQL 内部编码成本，也不替代完整文件 50 MiB 门禁。没有 KMS lease、短期下载授权、保留扫描、账号擦除协调或用户入口，不能把 `available` 状态用于任何真实请求。
- 训练所有权导出包含当前关系图、软删除会话和完整修订历史。同步投影已用 UUID 补全同时间顶层顺序，父级唯一 position/revision 保证嵌套顺序；但一个现有契约允许的 30×50 当前关系图在不含历史时已经超过 64 KiB。把它作为一行会拒绝合法用户证据，提高门禁则会恢复无界 Node 分配。内部 `createWorkoutSetLayerSnapshot()` 只校验一次 active owner，以全历史三元 keyset 读取 15 个会话标量，为每个会话按 position 读取九个动作标量，再为每个动作按 position 读取十一个 set 标量；owner/幂等/请求哈希和父外键不进入 payload。每层必须恰好一次且完整读取，显式 `complete()` 前根事务保持打开；跳过、重复、提前停止或取消会按 set→exercise→root 清理并以同一根错误回滚。后续仍必须在该 owner 根事务中读取修订头并分解不可变 snapshot，不能丢失纵向更正历史。
- 内部 JSON 来源类型现在允许 workouts 内的 exercises、sets 和 history 使用显式私有懒节点。嵌套测试证明内层数据在字段到达前不读取，首个 set 后停止会先执行其来源清理，再以同一错误取消根生命周期。数据库已补上 workout→exercise→set 的同根查询和三段收据，但仍没有修订嵌套来源或完整 v4 组合，不能作为训练导出完成证据。
