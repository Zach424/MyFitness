...
The API and client share one exact 50 MiB (52,428,800-byte) synchronous-export boundary. After formatting the repeatable-read payload, the API measures the final UTF-8 string before allocating the attachment Buffer. An oversized result returns the stable 413 `portable_export_too_large` receipt without payload content; an accepted result declares the exact byte count as `Content-Length`. Transport completion is still not sufficient client evidence. A lazily loaded file adapter reads the H5 Blob or WeApp temporary file before any H5 download action or WeApp persistent save. A dependency-free verifier requires `application/json` (an optional charset is accepted), the exact four-field v4 envelope, the current data-collection key set, object/array topology, a valid offset generation time, UUID account identifier and the same UTF-8 boundary. Invalid, old-version, wrong-media, oversized or unreadable artifacts never enter success and use product-owned copy only. The page retains only schema version, generation time and byte length as its receipt; account ID and export content are not logged, persisted or rendered. Temporary H5 Blob URLs are revoked on failure and after download. WeApp production compilation proves adapter compatibility, while real-device temporary-file and saved-file behavior remains an explicit external gate.
...
- Export rows, the JavaScript object graph and the formatted JSON string are still generated in API memory. The server now rejects final UTF-8 output above the shared 50 MiB boundary before allocating an attachment Buffer, but this is not streaming or an async encrypted archive; Mini Program real-device download/read/save behavior is not yet exercised.
