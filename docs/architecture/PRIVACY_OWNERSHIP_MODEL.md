...
异步归档的数据库保管模型与同步下载保持分离。数据库流事务以只读 `REPEATABLE READ` 固定一个 active owner 快照，只有 JSON 物理 EOF 才提交；提前停止、查询错误或主动取消都会回滚。当前协调入口按 v4 顺序读取同意事件、健康记录、健康修订、两个 owner 自定义目录和完整 workouts。workout 会话复用协调根已有 `PoolClient` 与 owner 结论，不建立第二事实时刻；七层数据仍逐元素执行 64 KiB UTF-8 门禁。独立餐食会话已有递归 `nutritionMeals` JSON 适配器，适配器不预取、不复制正文，并把物理 EOF 或最深 item 早停直接委托给同一数据库生命周期；但它尚未复用协调根，所以不能证明餐食与前六字段同属一个事实时刻。真实 PostgreSQL 已分别证明六字段和独立餐食的完整字节一致及最深取消。认证加密与 multipart writer 已独立验证，但第七协调字段、后续集合/媒体、KMS、状态执行、签名下载和 UI 仍缺失，因此这不是可用归档声明。
...
- 异步归档目前已有共享状态/预约、所有者只读流事务、同意/健康/修订/两个目录/完整 workouts 六字段协调来源、64 KiB 门禁、根生命周期 JSON、认证加密和 multipart 写入。两个目录都含活动与归档 owner 条目而不含 starter；workout 按 JSONB `history→exercises` 在同一事实时刻读取。餐食已有 owner-safe 无正文 shape 收据、UUID 顶层总序、非部分索引和独立四层正文会话；递归 JSON 适配、第七字段、后续集合/媒体、KMS lease、状态执行、限时下载、保留扫描、账号擦除协调和用户入口仍缺失。
- 餐食所有权导出包含软删除 meal、当前 items 与全部完整修订快照。独立会话只验证一次 active owner，在同一事实边界内按 meal→current items→history→snapshot items 逐层读取，并让每个数据库 payload 先经过 64 KiB 门禁。五段收据不含 owner/meal/revision UUID 或 food 正文；非数组 snapshot、跨 owner、字段乱序、重复消费、早停和最深层取消都不能提交根事务。真实 PostgreSQL 已证明完整物化值与同步投影逐字节相同，但该独立事务尚不能证明与前六字段属于同一事实时刻。
