...
- 异步归档目前已有共享状态/最小收据、PostgreSQL 保管约束、所有者事务预约、只读流事务、首个健康记录有界 keyset 行源、可组合懒数组 JSON 编码、内部认证加密 codec 和有界 multipart 对象写入；健康记录已完成数据库→字段完整 v4 的惰性组合证明，但其他集合、嵌套聚合和媒体仍完整驻留，而且这些组件尚无受租约执行器统一状态。批次限制是行数上界，不是单个元素 JSON 字节上界。没有 KMS lease、短期下载授权、保留扫描、账号擦除协调或用户入口，不能把 `available` 状态用于任何真实请求。
