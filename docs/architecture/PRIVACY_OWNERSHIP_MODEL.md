...
异步归档的数据库保管模型与同步下载保持分离。`privacy_export_archives` 的六状态、RESTRICT owner 外键、触发器和内部预约服务固定意图、单调状态与所有者读取；确定性对象键和 key-ref 只留在服务端。内部加密 codec 使用分块 AES-256-GCM，并把格式、owner、archive、v4 与非秘密 key-ref 摘要纳入每帧 AAD；随机前缀加帧序号形成唯一 nonce，认证空 final 帧和物理 EOF 拒绝截断/尾随。解析器在读取帧长度前先限制头部声明块，单帧认证后才输出明文，完成收据只在整个输出消费完毕后提供密文 SHA-256/大小。该收据是未来执行器内部证据，不进入公共最小收据。当前没有 KMS 数据密钥 lease、JSON 流式快照、公开预约路由、生成任务、归档对象、签名下载端点或客户端入口，因此 codec 仍不是可用功能声明。
...
- 异步归档目前已有共享状态/最小收据、PostgreSQL 保管约束、所有者事务预约和内部认证加密 codec；没有增量 JSON 快照、KMS lease、生成执行器、流式对象写入、短期下载授权、保留扫描、账号擦除协调或用户入口，不能降低当前同步导出压力，也不能把 `available` 状态用于任何真实请求。
