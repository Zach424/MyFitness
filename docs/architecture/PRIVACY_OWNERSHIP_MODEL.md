...
异步归档的数据库保管模型与同步下载保持分离。数据库流事务以只读 `REPEATABLE READ` 固定一个 active owner 快照，只有 JSON 物理 EOF 才提交；提前停止、查询错误或主动取消都会回滚。当前协调入口按 v4 顺序读取同意事件、健康记录、健康修订、两个 owner 自定义目录、完整 workouts、完整 nutritionMeals 和当前 nutritionFavorites。workout 与餐食会话复用协调根已有 `PoolClient` 与 owner 结论；收藏以既有 `(user_id,food_key)` 主键直接 keyset，不建立第二事实时刻。全部正文逐元素执行 64 KiB UTF-8 门禁。递归与普通 JSON 数组都不预取、不复制正文，物理 EOF 或任意活动字段早停直接委托给同一协调生命周期。真实 PostgreSQL 已证明八字段完整字节一致、并发收藏不可见及普通数组取消同根。认证加密与 multipart writer 已独立验证，但 weeklyPlans、AI/照片集合、媒体、KMS、状态执行、签名下载和 UI 仍缺失，因此这不是可用归档声明。
...
- 异步归档目前已有共享状态/预约、所有者只读流事务、同意/健康/修订/两个目录/完整 workouts/完整 nutritionMeals/当前 nutritionFavorites 八字段协调来源、64 KiB 门禁、根生命周期 JSON、认证加密和 multipart 写入。两个目录都含活动与归档 owner 条目而不含 starter；workout、餐食与收藏都在同一事实时刻读取。weeklyPlans、AI/照片集合、媒体、KMS lease、状态执行、限时下载、保留扫描、账号擦除协调和用户入口仍缺失。
- 餐食所有权导出包含软删除 meal、当前 items 与全部完整修订快照。协调会话只验证一次 active owner，在同一八字段事实边界内按 meal→current items→history→snapshot items 逐层读取，并让每个数据库 payload 先经过 64 KiB 门禁。五段收据不含 owner/meal/revision UUID 或 food 正文；非数组 snapshot、跨 owner、字段乱序、重复消费、早停和最深层取消都不能提交根事务。真实 PostgreSQL 已证明前六字段后的并发餐食新增不可见，完整物化值与同步投影逐字节相同。
- 收藏所有权导出只包含当前 `(user_id,food_key)` 快照，不伪造已经不存在的取消历史。第八字段复用根 active-owner 结论，按 `food_key` keyset 交付同步十四字段对象；复合主键同时保证 owner 隔离与唯一总序。最大合法行低于 64 KiB，越界门槛前一字节会在正文交付前拒绝；根开始后的并发收藏不可见，活动收藏早停会让迭代器、JSON 与数据库收据共享同一错误。
- 周计划所有权导出必须覆盖当前计划、全部不可变 history、活动/已关闭 workout links 和每修订当前体验反思。无正文 shape 收据证明 owner 范围与结构，但合法当前 payload、单 revision、history 聚合和 closed links 聚合都能超过 64 KiB；因此 weeklyPlans 尚未进入协调根。同步 link 顺序已补 `(linked_at,id)` 并由迁移 0036 的非部分 owner/plan 索引支持；后续必须递归分解 current/revision 内容并保持三个子集合独立 keyset，不能截断、只导出活动关系或提高门禁。
- 训练所有权导出包含当前关系图、软删除会话和完整修订历史。同步投影已用 UUID 补全同时间顶层顺序，当前关系表的父级唯一 position/revision 保证嵌套顺序；但一个现有契约允许的 30×50 当前关系图在不含历史时已经超过 64 KiB。把它作为一行会拒绝合法用户证据，提高门禁则会恢复无界 Node 分配。内部 `createWorkoutRevisionHeaderLayerSnapshot()` 只校验一次 active owner，以全历史三元 keyset 读取 15 个会话标量，为每个会话按 position 读取九个动作和十一字段 set，再在关系图完整结束后按 revision 读取 `id,action,revision,changed_at`。owner/幂等/请求哈希和父外键不进入 payload；history 查询同时验证父 workout owner 和冗余 revision owner。每层必须恰好一次且完整读取，显式 `complete()` 前根事务保持打开；乱序、跳过、重复、提前停止或取消会关闭最深活动来源，并以同一根错误回滚。
- 单 revision 的 `inspectWorkoutRevisionSnapshotShape()` 只返回兼容分类、计数、字节数、顺序布尔与 `decomposable`，不泄露账号、workout/revision UUID 或训练正文。它沿父链复核 owner 和 snapshot 身份，以严格允许键拒绝静默丢字段，并在 PostgreSQL 内证明根骨架、动作骨架和 set 各自不超过 64 KiB。shape 还要求 exercise/set UUID 在相应父级范围唯一，防止 ordinality 锚点歧义。历史数组的 JSON ordinality 属于不可变证据：合法反序 position 只令顺序标志为 false，不能成为重排或拒绝理由。
- `createWorkoutRevisionSnapshot()` 只在 shape 通过后读取同一事务中的正文；根和动作先交付保留键位的空数组骨架，再原地替换为懒来源。会话不把 UUID、正文或 ordinality 放入完成收据；不可分解、跨 owner 和取消错误都是固定无内容边界。snapshot、exercise、set 必须恰好一次且完整结束，主动取消先关闭最深活动 set，再关闭 exercise 和根事务。
- 内部 `createPortableExportWorkoutJsonSource()` 只接受 owner-scoped 七层数据库会话，把 workout、当前动作和修订快照中的数组原位包装为私有懒节点，并直接透传数据库收据、完成和取消。单元证明 history 在当前关系前且无预取，真实 PostgreSQL 证明完整数组逐字节等价和活动不可变 set 的同根失败。该来源没有进入公开 v4，也不能跨过 exercise/food catalog 直接附加到三字段协调器。
