...
第 196 轮的客户端读取层继续保持上述差异。传输函数只接受共享 subject 类型，成功正文必须再次通过严格视图 Schema 且 subject 与请求一致；扩展或畸形响应失败，HTTP/网络错误保持原样，不能转为 `current:null`。页面内存读取权限在请求开始前显式为 unread，开始后才进入初始加载；成功的空主题是 ready 快照，初始失败没有快照，刷新失败保留整份旧快照并成为 stale。每次读取拥有单调 generation 收据，旧成功/失败不能覆盖新请求；更换主题和页面卸载同时清空旧快照并使待处理回调失效。当前还没有展示组件或页面接线，也没有本地持久化、轮询和后台自动重试。
...
下一轮只设计并实现当前主题最小展示组件和纯展示模型：使用中性中文解释系统观察、本人确认、资料不足、用户不同意和终态，证据数量必须与观察窗口、截止时间和限制同时呈现；不生成完成率、依从率、能力评级或行动建议。先不接路由、不批量读取三个主题，也不开放反馈、lineage 或证据分页。Weekly Cognitive Review、模型导出与最终页面接线仍拆到后续轮次。
