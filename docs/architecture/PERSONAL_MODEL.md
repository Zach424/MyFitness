...
下一轮只实现一个固定当前主题的独立页面读取闭环：进入时读取，复用现有 unread/五阶段/代次失效与中性卡片，公开刷新、初始失败重试和 stale 提示，卸载时使在途结果失权。先不批量读取三个主题，也不开放反馈、lineage 或证据分页。Weekly Cognitive Review、模型导出与其余页面接线仍拆到后续轮次。
