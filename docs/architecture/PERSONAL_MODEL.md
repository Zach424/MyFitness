...
状态：第 202 轮完成三种本人校准与成功后重读；temporary 截止时间、回顾持久化、模型导出和完整客户端闭环仍未实现
...
第 190 轮为 `recorded_training_frequency_behavior_v1` 增加首个纵向确定性执行路径。账户建立时间只界定数据库能够观察的最早时点；派生器使用当前 profile 的 IANA 时区，排除建立账户所在的不完整首周和当前进行中的周，然后取最近至多 8 个完整本地周。跨 DST 的周以本地周一边界转换成精确时刻，不假定每周恒为 168 小时。来源只接受窗口内当前未删除 workout 的精确 revision，同一训练出现多个当前修订、周位置不一致或窗口外证据均失败关闭。

没有完整周时返回 `insufficient_coverage` Unknown；存在完整周但没有当前合格训练时返回 `no_eligible_evidence` Unknown。两者都不创建“零训练”Behavior。出现正向记录但窗口少于 4 周或合格训练少于 6 次时只创建 low-confidence candidate；同时满足两项门槛才创建 moderate active。当前只比较一个滚动窗口，因此即使 active 也保留 `single_window` limitation、`comparedWindowCount=0` 与 `stableWindowCount=0`，不能据此宣称稳定习惯、依从性或现实总训练量。

同一完整窗口和精确来源语义返回 no-op；窗口推进或训练更正会重新计算每周课次、记录周数、中位数和范围。旧 confirmed/uncertain 不转移到新 claim，用户 disagreement 继续 disputed，仍有效的 temporary 才保留。最后一条当前证据删除时，旧 Behavior 追加 `invalidated` revision、全部旧引用转为 withdrawn，并同时返回 Unknown 收据。终态收到迟到来源义务时只追加同终态撤回历史以解决请求，不采用新 claim，也不复活条目。

repository 在 READ COMMITTED 事务中先锁 active owner，再读取 item/current/pending；随后用一条 SQL 从账户、当前 profile 和当前 workout/history 生成评估时刻、完整本地周边界与有序快照，避免应用分段观察混用不同来源视图。owner 锁串行化同账户的两个派生执行者；训练在观察并发布之间变化时，来源资格提交门禁会拒绝旧证据，训练在提交后变化则由 refresh request 留下持久义务。该执行器仍无控制器、worker、公开 API 或客户端入口。

这里的覆盖语义必须用产品语言解释清楚。账户建立只说明系统在该时刻以前不可能拥有本账户的训练记录，不能反过来证明此后的记录没有遗漏。完整周只解决窗口长度可比较的问题，也不证明用户每次现实训练都打开了产品。因此“无合格证据”只允许显示为未知，“每周零次”只会出现在已经存在其他正向记录、且该周属于明确完整观察窗口的统计数组中；即使如此，界面未来也必须继续标注它是记录结果，不是现实活动判定。

候选与有效状态的区别同样只是内部使用资格，不是对人的评价。四个完整周和六次训练是当前保守工程门槛，用来阻止极少量记录立即参与低风险建议；它没有经过真实人群校准，也不代表训练是否足够、规律或健康。有效条目仍携带单窗口限制，后续只有经过多个非重叠窗口、本人校准和结果回看，才可以讨论更长期的描述，而且任何时候都不得把次数高低转写成意志、性格或依从性标签。
...
第 196 轮的客户端读取层继续保持上述差异。传输函数只接受共享 subject 类型，成功正文必须再次通过严格视图 Schema 且 subject 与请求一致；扩展或畸形响应失败，HTTP/网络错误保持原样，不能转为 `current:null`。页面内存读取权限在请求开始前显式为 unread，开始后才进入初始加载；成功的空主题是 ready 快照，初始失败没有快照，刷新失败保留整份旧快照并成为 stale。每次读取拥有单调 generation 收据，旧成功/失败不能覆盖新请求；更换主题和页面卸载同时清空旧快照并使待处理回调失效。

第 197 轮增加未接线的纯展示模型与 props-only 卡片。训练安排只标为本人提交的可安排时间，不冒充完成；频次与时长都保留“已记录”限定，并补充未记录现实训练不在范围、时长不评价效果。空主题明确不是零或已了解；candidate、active、disputed 和两个终态分别说明能否作为计划依据，本人核对状态与系统状态并列而不互相覆盖。资料覆盖不用数值置信分或进度条，而是中文等级、限制列表和合格/支持/冲突/撤回的精确计数，并始终附带观察窗口、截止时间、代次/修订和有效期。组件没有 API、路由、按钮或本地状态，仍没有页面接线、持久化、轮询和后台自动重试。
...
第 202 轮把“符合我”“不同意”“暂不确定”三种完整选择接入当前主题页面。反馈只对非空、非终态 ready 快照开放，提交期间冻结主题、item、revision、event 与 choice；网络未知或暂时服务失败只允许本人点击同 event 重试，409 只允许先读取新修订。成功后页面重新请求完整 current view，只有 item、current revision 与反馈状态一致才确认已经重新核对，最小写收据不会被当成新的完整认识。

“只是暂时情况”继续可见说明但尚不可提交，因为共享契约要求本人给出未来截止时间，客户端不会填入隐含默认期限。reason、note、历史和回顾同样未开放。页面沿用证据核对表的紧凑签署样式，不生成评分、完成度、情绪颜色或自动计划变化。本地成功路径已证明 `uncertain` 从 R1 写入 R2 并完成重读；同时发现合法新建档 goal 在生成 `training.availability` 时被来源资格门禁错误拒绝，下一轮必须先修复该失败关闭缺陷。

换言之，读取成功只说明系统找到了当前保留的一代认识，读取为空只说明该主题尚无条目；两种结果都不能被解释为现实行为存在、缺失、达标或失败。

未来界面展示这项视图时，应先说明“这是系统根据已记录资料整理的当前认识”，再呈现认识内容、资料覆盖时段、更新时间、限制和本人反馈状态。界面不得把状态翻译成能力等级、健康评级、训练成绩或执行好坏，也不得因为认识处于有效状态就省略限制。终态认识仍要使用中性的“已失效”或“已由后续认识替代”语义，避免让用户误以为自己失败、违规或身体发生了确定变化。

证据摘要中的数量只表示这次派生所纳入、支持、反对或已经撤回的记录引用数量，不表示现实中发生行为的总次数。资料覆盖不足、记录中断、设备漏记和用户选择性记录都会改变它能代表的范围；因此数量必须和观察窗口、计算截止时间及限制一起出现，不能单独转化成完成率、依从率、风险分数或建议强度。具体证据正文另有更高权限的审计边界，当前可见视图不能为了增加“解释感”而重新拼装或猜测原始记录。

传输中的稳定条目标识只用于后续把反馈精确送回同一项认识，普通界面无需直接显示。页面也不能根据数字修订或代际大小生成“越来越好”“越来越差”之类的排序含义；它们只负责防止反馈落到错误版本，并帮助系统区分当前认识与历史认识。若后续开放历史浏览，必须独立设计分页、每代时间语义、终态原因和证据权限，不能把当前视图在客户端递归扩展成历史链。

不可用与空主题必须提供不同的运行语义，但对用户的文案仍要克制。空主题可以说明“目前还没有足够流程形成这项认识”，不可用则只能表示当前无法安全读取并允许稍后重试，不能透露账号是否存在、是否正在删除或内部数据是否发生冲突。内部数据冲突必须进入受控日志和运维处理，不得在客户端退化成空卡片，也不得把数据库错误正文带入界面。
...
这些路径尚未进入 OpenAPI。当前只有按 owner/subject 读取唯一未退役代的内部 repository 信封，携带 generation、直接前代、terminal、固定未退役元数据与完整 revision；它不等于公开响应，也不提供 lineage 列表。实施时先定义公开最小投影和授权错误，再开放 API、分页与客户端；不得在一轮同时完成全部层。
...
| P5 周回顾与反馈        | 三种本人校准、同 event 恢复与成功后重读已完成；temporary 与周回顾待续            | 精确 revision、显式重试、冲突重读与客户端权限通过                          |
...
下一轮优先修复真实新建档 goal 无法生成 `training.availability` 的精确来源资格缺陷：用真实引导资料建立 PostgreSQL 回归，证明首次派生、刷新、owner 隔离与账户删除。之后再设计 temporary 未来截止时间选择器；可选原因、敏感备注、lineage、证据分页、Weekly Cognitive Review 和模型导出继续后置。
