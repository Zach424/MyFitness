...
这里的“当前”只代表某个条目已经发布的最新修订，不代表系统掌握了用户完整、永久或唯一的真实状态。第 193 轮的内部 `personal-model-current-subject-envelope-v1` 进一步把“当前主题”限定为 active owner 下唯一 `retired_at IS NULL` generation 的精确 current revision；空主题明确为 null。terminal 表示 revision 已 superseded/invalidated，retired 表示该代已被后继替换，两者不能互换。数据库可以证明一条历史属于谁、前后顺序是否连续、内容是否被直接改写，却不能证明观察是否充分、结论是否准确或用户是否认同。后续反馈、来源撤回和回顾流程必须继续保留这些不确定性，任何展示层都要让用户看见依据、时间、限制与更正入口。
...
下一轮进入 P4 的公开读取前置：从严格内部信封设计当前主题的用户可见最小投影与授权错误边界，明确可展示的状态、时间、限制和空结果，并禁止跨 owner、历史代或内部证据正文泄露。先不一次完成 HTTP、分页、客户端或完整 lineage 列表。Weekly Cognitive Review、模型导出与客户端仍拆到后续轮次。
