...
| R-009 | H5 入口仍超过 webpack 建议警告值，双端包体余量仍需持续保护                      | 中   | H5 入口/最大异步块为 315,266/152,167 字节；WeApp 总量为 1,108,916/1,109,000 字节且只余 84 字节。不得提高预算；封闭测试前仍须在代表性浏览器与微信真机复核加载、缓存和分包行为 |
...
| R-032 | 个人状态证据账本可能被误解为完整、实时或统一的个人真相                          | 高   | 固定知识类别、权威来源、快照时间、无保证有效期和失效原因；账本不持久化、不诊断、不自动适应。下一轮补来源追溯，封闭测试前用真实用户验证类别、遗漏和时效理解                   |
