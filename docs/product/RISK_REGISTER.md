...
| R-029 | 客户端发生时间转换依赖平台 `Intl` 时区数据                                      | 中   | 间隙/重叠/未来时间、洞察绝对时间和服务器营养 90 日本地日门禁已通过，但不能替代真实平台 IANA 证明；封闭测试前跨代表性时区验证受支持 H5 浏览器和真实微信设备                      |
