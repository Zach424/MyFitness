...
第 101 轮仅复核 R-009 的本地包体证据；第 102 轮新增 R-030，以记录主观恢复估计尚未完成真实纵向校准；第 103 轮新增 R-031，并按结果回看的实际 WeApp 交付成本重基线本地包体预算。全表最近一次完整审阅仍是上述第 043 轮。
...
| R-009 | H5 入口仍超过 webpack 建议警告值，双端包体余量仍需持续保护                      | 中   | H5/WeApp 入库预算、照片用途路由拆分和异步公共块已通过；封闭测试前在代表性浏览器与微信真机复核加载和分包行为                 |
...
| R-031 | 固定七天计划结果窗口和用户主动记录存在选择偏差，可能被误解为计划效果            | 高   | 只显示精确修订、时间窗和当前确认来源，Unknown/Observed 不推断因果、依从性或自动适应；封闭测试前用真实纵向数据评估覆盖与窗口 |
