...
| R-032 | 个人状态证据账本可能被误解为完整、实时或统一的个人真相                          | 高   | 固定知识类别、权威来源、快照时间、无保证有效期和失效原因；卡片可返回既有计划/记录/历史权威并在返回后显式刷新，但不把导航冒充新鲜证据。封闭测试前仍须用真实用户验证类别、遗漏和时效理解 |
