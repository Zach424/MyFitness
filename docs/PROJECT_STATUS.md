...
Stage: hosted source/image CI green; immutable candidate release promotion in local acceptance before managed shared deployment
...
| Delivery roadmap        | Done for planning baseline   | `docs/product/ROADMAP.md`                                               | Publish iteration 19 candidate              |
...
| Testing/observability   | Partial                      | 111 unit tests including release invariants; hosted quality/smoke green | Centralize alerts/tracing                   |
| Deployment              | Partial, promotion ready     | OCI topology green remotely; strict three-image release record locally  | Publish candidate, then managed environment |
...
| First real GHCR candidate, release assets and registry pull are not yet observed       | Medium | Publish `v0.1.0-rc.1`, verify the downloaded manifest/checksum, attestations and digest pulls       |
...
Iteration 19: complete the immutable three-image release promotion, wait for the implementing main CI, publish `v0.1.0-rc.1`, download and verify its checksummed manifest, and prove digest-qualified registry access. Iteration 20 then provisions the repeatable managed shared test environment with approved account/region/budget, secret injection, real WeChat/OIDC credentials, domain allow-listing, centralized telemetry/alerts, calibrated edge limits and manifest-driven deploy/rollback.
