...
| Design language         | Partial, nineteen flows tested  | Core flows + 320 px/large-text/keyboard proof + 36 reviewed screenshots | Screen readers and remaining state matrix   |
...
| Domain rules            | Partial                         | Stable exercise/food definitions snapshot into correction-safe facts    | Daily nutrition observation                 |
...
| Testing/observability   | Partial                         | 221 unit, 57 integration, 30 browser tests + client quality gate        | Centralize telemetry after account approval |
...
| H5 entry remains 306 KiB (above webpack's 244 KiB advisory); largest async route is 190 KiB | Medium | Enforced 320/200 KiB, 2.00 MB H5 and 725 KB WeApp budgets prevent growth; split entry before beta           |
...
Iteration 40: add a timezone-aware daily nutrition observation from confirmed, current, non-deleted meal snapshots. Provide bounded 7/30/90-day daily energy/P/C/F/fiber and meal-count evidence, explicit missing days, correction/deletion recomputation and a compact H5/WeApp view. Do not compare users, score food quality, infer adherence, set targets or recommend intake. Owner-operated cloud account/budget, domain/TLS, real WeChat/OIDC, data-custody owners, centralized telemetry, policy/filing decisions and any approved paid-provider canary remain mandatory external inputs but are parked until the user supplies them; do not describe local MinIO, fixture AI or candidate artifacts as public production.
