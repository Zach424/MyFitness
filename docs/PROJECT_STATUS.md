...
Stage: internal alpha locally runnable; the workout loop now moves from a versioned starter/owner-created exercise definition to immutable completed-set evidence and exact stable-key 7/30/90-day observation. Same-name different-key movements remain separate, incomplete sets never inflate metrics, recorded display/equipment snapshots stay visible and workout correction/deletion recomputes the current projection without duplicate storage. Explicit plan-to-workout links, bounded plan freshness, measured H5/WeApp budgets, 320 px/large-text/keyboard proof and full-runtime scanning remain green. Next 16.2.11 plus patched parser/glob floors retain zero critical/high production audit findings. Privacy-first progress photos, both client candidate contracts, H5 OIDC trust, immutable workflow dependencies, exact release-source qualification, recoverable erasure, crash-safe AI and reproducible evaluation artifacts remain locally green. Owner-operated cloud, domain and real-provider work is explicitly parked while locally verifiable product gaps continue.
...
| Design language         | Partial, eighteen flows tested  | Core flows + 320 px/large-text/keyboard proof + 35 reviewed screenshots | Screen readers and remaining state matrix   |
...
| Domain rules            | Partial                         | Workout authority + stable-key completed-only exercise observations     | User-owned food facts and provenance        |
...
| Testing/observability   | Partial                         | 217 unit, 55 integration, 29 browser tests + client quality gate        | Centralize telemetry after account approval |
...
| H5 entry remains 306 KiB (above webpack's 244 KiB advisory); largest async route is 190 KiB | Medium | Enforced 320/200 KiB and 1.85 MB H5 budgets prevent growth; split app/Taro entry before beta             |
...
Iteration 39: add an owner-created food catalog for foods missing from the demonstration starter set. Store explicit per-100-g nutrients and visible user-confirmed provenance; support idempotent creation, optimistic correction, archive and immutable revisions; snapshot the selected definition into meals; and include definitions/history in privacy inventory, portable export and erasure. Never relabel user-entered nutrients as verified provider facts or turn them into intake prescriptions. Review external food datasets only as governed license/localization/provenance candidates; do not bulk-import content this round. Owner-operated cloud account/budget, domain/TLS, real WeChat/OIDC, data-custody owners, centralized telemetry, policy/filing decisions and any approved paid-provider canary remain mandatory external inputs but are parked until the user supplies them; do not describe local MinIO, fixture AI or candidate artifacts as public production.
