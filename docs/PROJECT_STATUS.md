...
Stage: verified WeChat identity complete locally; shared test deployment and H5 identity next
...
| Delivery roadmap        | Done for planning baseline   | `docs/product/ROADMAP.md`                                            | Execute iteration 17                        |
...
| Business API            | Partial                      | Verified WeChat adapter, shared edge, operator boundary and jobs     | Shared deployment and real credential proof |
...
| Testing/observability   | Partial                      | 100 unit + 42 integration + 7 worker + 21 E2E; high-risk audit clear | Centralize scraping, alerts, tracing and CI |
...
Iteration 17: create a repeatable shared test deployment with managed data services/secrets, real WeChat credentials and domain allow-listing, centralized telemetry and alerts, calibrated edge limits, plus exercised deploy/rollback. Carry every identity and data-custody gate forward; a local mock provider, MinIO and restore drill are not production approval.
