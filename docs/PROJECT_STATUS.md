...
Stage: internal alpha locally runnable; the workout loop now combines a versioned starter catalog with owner-created exercises, searchable aliases and explicit tracking/equipment semantics. Definition correction/archive preserves immutable catalog revisions and never rewrites a selected draft or saved workout snapshot; cross-owner, duplicate, stale and idempotency-conflict writes fail at the server, and catalog data joins portable export/erasure custody. Explicit plan-to-workout links, bounded plan freshness, measured H5/WeApp budgets, 320 px/large-text/keyboard proof and full-runtime scanning remain green. Next 16.2.11 plus patched parser/glob floors retain zero critical/high production audit findings. Privacy-first progress photos, both client candidate contracts, H5 OIDC trust, immutable workflow dependencies, exact release-source qualification, recoverable erasure, crash-safe AI and reproducible evaluation artifacts remain locally green. Owner-operated cloud, domain and real-provider work is explicitly parked while locally verifiable product gaps continue.
...
| Design language         | Partial, seventeen flows tested | Core flows + 320 px/large-text/keyboard proof + 34 reviewed screenshots | Screen readers and remaining state matrix   |
...
| Domain rules            | Partial                         | Workout authority + revision-bound links + exercise snapshot semantics  | Exercise-level history/trend projection     |
...
| Testing/observability   | Partial                         | 212 unit, 54 integration, 28 browser tests + client quality gate        | Centralize telemetry after account approval |
...
Iteration 38: add an exercise-level history and trend projection over stable catalog keys and completed-only workout evidence. Keep display/equipment facts tied to each recorded snapshot, avoid merging different custom identities by name, show missing evidence instead of invented progress, and do not turn volume or personal bests into automatic progression advice. Prove correction-safe grouping and timezone/order behavior in PostgreSQL plus H5/WeApp. Owner-operated cloud account/budget, domain/TLS, real WeChat/OIDC, data-custody owners, centralized telemetry, policy/filing decisions and any approved paid-provider canary remain mandatory external inputs but are parked until the user supplies them; do not describe local MinIO, fixture AI or candidate artifacts as public production.
