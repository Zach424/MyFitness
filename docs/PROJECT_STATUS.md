...
Stage: first service candidate published; H5 OIDC server trust, immutable workflow dependencies, exact tag/main/CI release-source qualification, deterministic client release, combined admission, recoverable erasure receipts, crash-safe AI lifecycle, adversarial output validation and reproducible evaluation artifacts are locally green before the next candidate and external shared infrastructure
...
| Delivery roadmap        | Done for planning baseline      | `docs/product/ROADMAP.md`                                            | Execute iteration 29 H5 browser identity    |
...
| Client: Mini Program/H5 | Partial                         | WeApp candidate; H5 server OIDC ready but browser TAR preview-only   | Add callback/PKCE client and release mode   |
...
| Testing/observability   | Partial                         | 157 unit, 47 integration, 22 browser tests + eval/action drift gates | Green implementing CI; centralize telemetry |
...
| WeChat lacks real device/domain proof; H5 OIDC lacks browser/provider proof          | High   | Add state/nonce/S256 callback, then exercise both adapters and explicit re-registration policy           |
...
Iteration 29: implement the H5 browser half of ADR-0027 with cryptographic state/nonce/verifier generation, PKCE S256, tab-scoped transaction storage, exact callback validation/cleanup, explicit login/error UI and an `oidc / candidate` deterministic H5 release contract. Then obtain the approved API URL and managed-environment inputs for a no-traffic deployment round; public delivery remains held until real provider, DNS/TLS/CORS and browser evidence pass.
