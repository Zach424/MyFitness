...
- AI explanation lifecycle status/reconcile: `GET/POST http://127.0.0.1:3100/v1/internal/ai-explanations[/reconcile]`
...
`/internal/metrics`, `/internal/data-operations` and `/internal/ai-explanations` require `x-operations-token` and are not administrator APIs. Metrics return no-store Prometheus text; data operations return aggregate state/age or a bounded drain result only; AI lifecycle operations return aggregate counts/timing or one bounded reconciliation count only. Direct users, run/job identifiers, explanation content, prompts, contexts, object keys, receipt secrets, IPs, request IDs, query values, tokens and health payloads are excluded. Production requires private scraping/control paths plus Redis TLS/ACL configuration and exact reverse-proxy hop trust.
...
Weekly-plan generation requires `x-idempotency-key` and a Monday `weekStart`. An unchanged profile/week returns the current aggregate; a changed onboarding revision regenerates that aggregate as a new draft revision. The list route attaches a read-only server freshness projection from current onboarding revision and eligibility: only `current` permits accept/modify and AI explanation, while `profile_changed`, `eligibility_blocked` and defensive `onboarding_required` retain skip plus a bounded recovery action. This projection is not written into the aggregate or immutable snapshots. Decisions carry `expectedRevision`; `modify` may submit only contract-listed substitutions, while `accept`, `modify` and `skip` append immutable snapshots. Generation and actionable decisions independently re-check current professional-clearance eligibility. The deterministic plan contains general training and qualitative meal/hydration focuses, not medical or calorie prescriptions.

AI explanation generation also requires `x-idempotency-key`, the exact `expectedPlanRevision`, and affirmative purpose/version consent. It re-checks ownership, current onboarding and risk eligibility, then sends only a minimized structured plan summary to the internal worker. Every pending reservation has a database deadline beyond the worker timeout and a validated recovery result; startup/interval reconciliation atomically converts expired rows to the same visible deterministic fallback returned by an idempotent retry. New calls require validator v2, which normalizes Unicode/format/separator obfuscation before medical, prescriptive, numeric-grounding and instruction-leakage checks; history responses retain readable v1/v2 provenance. The response always identifies `model`, `fixture`, or `fallback` source plus prompt/validator/model provenance; it never mutates the weekly plan. Local Compose defaults to `AI_PROVIDER=fixture`. Enabling `openai` additionally requires `OPENAI_API_KEY` and release approval for provider privacy, region, cost, latency and quality gates.

Food-photo reservation requires `x-idempotency-key` and current affirmative purpose/version consent. The signed upload accepts one bounded JPEG/PNG/still WebP multipart `file`; the API re-encodes it before private S3-compatible storage or worker access. Prompt v2 treats text visible inside the image as untrusted data; validator v2 applies the same adversarial display-copy policy before catalog/range confirmation. History accepts v1/v2 provenance during rolling upgrades. Candidates expose ranges and provenance, never create a meal, and confirmation accepts only displayed catalog keys/grams before transactionally enqueueing media deletion and returning unsaved draft inputs. Logical deletion and `mediaDeletionStatus` are separate. The signed preview is short-lived and is not a public asset URL.
