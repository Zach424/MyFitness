...
- 导出接口以附件 JSON 返回；API 与客户端共享 50 MiB（52,428,800 字节）上限。API 在创建附件前按最终 UTF-8 序列化字节检查，超限返回 413 `portable_export_too_large`；客户端仍独立校验 `myfitness-portable-export-v4`、媒体类型、结构和同一上限后才落盘。
...
| 413             | 上传字节超限，或同步便携导出超过 50 MiB  | 图片重新选择/压缩；导出客户端仅依状态码显示产品自有说明，不展示响应正文、盲重试或标记已下载，后续改用归档流程 |
...
| `GET /v1/insights/health/{metric}`         | path HealthMetric | 固定窗口、最多 180 个携带精确 metric 的点、hasMore  | 400、401 |
...
契约不变量：窗口必须严格按 `[7,30,90]`；动作/健康来源窗口行只能使用这三个身份且不得重复，缺失行仍生成空窗口，输入顺序不影响响应；来源计数必须是安全非负整数，修订和点总组数为正安全整数，训练测量为有限非负数，健康统计/记录值为有限数且不做医疗正负判断；动作窗口课次不超过完成组数，零课次不得携带完成组，零完成组时次数/负荷/活动时间/距离必须全零，有完成组时这些测量仍可为零；动作点满足 `0 < completedSetCount <= totalSetCount`，健康窗口把记录数/记录日/统计存在性绑定且满足 `minimum <= average <= maximum`；动作窗口按 7→30→90 日要求 `sessionCount`、`completedSetCount`、`totalReps`、`volumeKg`、`activeMinutes` 与 `distanceKm` 全部非递减；健康窗口同样要求记录数/记录日非递减，较长窗口 minimum 不高于非空较短窗口、maximum 不低于非空较短窗口，average 不受单调约束；动作 90 日窗口和点查询必须共享资格边界，`sessionCount <= 180` 时来源行数与课次计数相等，`sessionCount > 180` 时来源恰有 181 行；公开 `series.length` 必须等于 `min(sessionCount,180)`，`hasMore` 必须等于 `sessionCount > 180`，缺失 90 日来源只在零点时回退为空；未截断动作窗口的完成组/次数必须精确等于完整点总和，负荷/活动时间/距离必须落在逐点显示舍入的计算上界内，截断时不执行完整聚合对账；健康 90 日窗口和点查询同样共享资格边界，`recordCount <= 180` 时来源行数与计数相等，`recordCount > 180` 时来源恰有 181 行；公开 `series.length` 必须等于 `min(recordCount,180)`，`hasMore` 必须等于 `recordCount > 180`，缺失 90 日来源只在零点时回退为空；动作来源身份必须使用 1–80 字名称、已知类别/追踪/器械、至多六项且不重复的器械，`other` 必须带 1–120 字备注；旧快照仍允许可空追踪模式与空器械；健康 SQL/来源行和每个公开点必须显式携带与路径/顶层完全相同的精确 `metric`，未来点与隐藏第 181 点同样受检，不从单位或数值反推身份；健康来源点还必须使用该指标规定的规范单位与允许展示单位、严格有界来源元数据和有效 IANA `recordTimezone`，confirmed-only 投影拒绝 `ai_estimate`；单位关系成立后，来源构造与响应 Schema 分别按 `NUMERIC(14,4)` 的双列舍入误差验证 `displayValue/displayUnit` 与 `canonicalValue/canonicalUnit` 属于同一测量，错配不自动修复；记录时区不参与响应本地日派生，换算门禁不重跑数值范围或改写展示值；参考时刻与来源点必须是有效时刻，点不得晚于 generatedAt；本地日期必须与 occurredAt 在响应时区中一致；动作/健康来源点必须先提供有效聚合 UUID，再按 occurredAt 非递增且 UUID 唯一，相同时刻允许、重复身份不去重；动作顶层 `identity` 与首点快照一致，健康顶层和全部点使用首点 `canonicalUnit`，空序列时两个摘要均为 `null`；空证据保持 null 而非零。

健康完整统计收据：当 90 日 `recordCount <= 180` 且点数收据成立时，来源 minimum、maximum、average 必须与全部 `canonical_value` 点集一致，只容纳十进制转 `Number` 后按幅度、绝对值和与点数计算的 IEEE 误差；公开 `statistics` 保留四位小数，允许 0.00005 半单位再叠加同形式浮点误差。`recordCount > 180` 时来源与响应都跳过该收据，不从 180 点前缀复算窗口。冲突失败关闭并定位精确统计字段；负值合法，不自动改值或生成范围、趋势、医疗解释。

健康完整记录日收据：同一未截断前提下，90 日 `recordedDays` 必须精确等于全部点按响应 `timezone` 投影后去重的自然日数量。来源使用与 SQL `occurred_at AT TIME ZONE` 等价的 IANA 日期格式化，响应在逐点 `localDate` 已验证后对集合计数；同日多点和夏令时重复小时只算一个自然日。`recordCount > 180` 时两层都跳过，不从有限前缀复算；冲突定位 90 日 `recordedDays`，不自动修改窗口或点。
...
- `GET /v1/personal-model/subjects/{subjectKey}/current` 是首个已实现 Personal Model 公开读取。它要求现有 Bearer 会话；`subjectKey` 只允许 `training.availability`、`training.recorded_frequency`、`training.recorded_session_duration`。200 响应严格为 `personal-model-current-subject-view-v1`：active owner 没有该主题时 `current:null`；非空时只保留当前 item/revision 数字定位、generation、结构化 claim、状态/反馈、置信限制、证据计数/窗口和必要时间，明确排除 owner、前代、内部 revision/reference UUID、EvidenceReference 正文、指纹与策略字段。无 Bearer 返回 401，非法 subject 返回 400，认证后 owner authority 竞态统一返回无身份线索 404，数据库歧义/损坏返回不带内部正文的 500；所有这些状态都固定 `Cache-Control: private, no-store`。当前 OpenAPI 仍不包含 Weekly Cognitive Review、lineage、证据分页或模型导出；当前读取与反馈服务不能被描述为完整认知镜子。
- `POST /v1/personal-model/items/{itemId}/revisions/{revision}/feedback` 要求 Bearer，并把 principal、UUID item 和正整数 current revision 绑定到 `personal-model-feedback-write-request-v1`。正文只接受 event UUID、四选一 choice、可空 reason/note 与 temporary 专用未来有效期；同 event 精确重试返回首次 `personal-model-feedback-write-response-v1`，换内容或过期/终态 target 返回 409。跨 owner、item 不存在和 authority 失效统一为无身份线索 404；非法路径/正文为 400，无认证为 401，内部错误为 500，全部 `private, no-store`。最小 200 响应只含 revised/no_op、event/item/target/current revision、choice、当前反馈/状态/有效期、服务器接受时刻和 no-op 原因，不返回 owner、claim、证据、note/reason 或指纹。
- 客户端读取传输只接受 `PersonalModelSubjectKey`，使用现有认证请求；成功 unknown 值由无 Taro、无 Zod 运行时的轻量严格核心再次解析，并要求响应 subject 与请求一致。反馈写入传输只接受精确 item/revision 与共享最小请求；调用前复核路径目标和正文，成功 unknown 收据同时通过轻量严格核心与请求身份核对。两个校验器共享严格公历/偏移时间守卫，拒绝运行时会自动归一化的非法日期；合法/非法夹具结果必须与权威 Zod Schema 一致。读取不会把 404、500、网络异常或畸形正文转成空主题；写入不会把 409、网络未知或畸形正文转成成功，也不会自动重放。
- 客户端反馈写权限以单调 generation 收据绑定 `subjectKey + itemId + revision + eventId + choice`，区分 `idle/submitting/succeeded/failed`。新提交、主题切换与页面失效会撤销旧收据；成功还必须与精确目标和选择一致。失败状态只保留 offline/refused/service/unknown/conflict/invalid-contract 分类，不保存服务端正文、note、reason 或请求副本。网络未知或暂时服务失败只允许本人明确触发同 event 重试，没有持久队列、定时器或自动重放。
- 当前主题中性展示只消费严格解析后的共享视图：空主题明确不是零，安排标为本人提交，频次/时长只描述已记录课次；状态、本人核对、资料覆盖/限制、四类精确证据计数、时域与代次/修订分开呈现。独立页面从“我的衡迹”可达，以三个 pressed 按钮逐项请求 `training.availability`、`training.recorded_frequency` 或 `training.recorded_session_duration`；一次只读取/持有当前选择，切换使旧请求、快照、失败和焦点失权，不批量预取或缓存组合画像。
- 非空、非终态 ready 快照可提交 `matches_me`、`disagree` 或 `uncertain`。写入期间主题、刷新和反馈均冻结；成功后显式重读当前主题，并用收据核对新的 item、current revision 与反馈状态。409 只允许先读取最新认识；最小写收据绝不直接替换完整 current view。`temporary_context` 仍需未来日期选择器，因此页面不会填入默认有效期；reason、note、历史、lineage 与回顾仍未接入。
