...
- 当前主题中性展示只消费严格解析后的共享视图：空主题明确不是零，安排标为本人提交，频次/时长只描述已记录课次；状态、本人核对、资料覆盖/限制、四类精确证据计数、时域与代次/修订分开呈现。首个独立页面固定请求 `training.recorded_frequency`，提供进入读取、手动刷新和失败重试，从“我的衡迹”可达；其他两个主题、批量读取、反馈和历史未接入，不扩大上述 HTTP 契约。
