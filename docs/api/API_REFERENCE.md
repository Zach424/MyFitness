...
契约不变量：窗口必须严格按 `[7,30,90]`；动作/健康来源窗口行只能使用这三个身份且不得重复，缺失行仍生成空窗口，输入顺序不影响响应；来源计数必须是安全非负整数，修订和点总组数为正安全整数，训练测量为有限非负数，健康统计/记录值为有限数且不做医疗正负判断；动作窗口课次不超过完成组数，动作点满足 `0 < completedSetCount <= totalSetCount`，健康窗口把记录数/记录日/统计存在性绑定且满足 `minimum <= average <= maximum`；动作来源身份必须使用 1–80 字名称、已知类别/追踪/器械、至多六项且不重复的器械，`other` 必须带 1–120 字备注；旧快照仍允许可空追踪模式与空器械；健康 SQL/来源行和每个公开点必须显式携带与路径/顶层完全相同的精确 `metric`，未来点与隐藏第 181 点同样受检，不从单位或数值反推身份；健康来源点还必须使用该指标规定的规范单位与允许展示单位、严格有界来源元数据和有效 IANA `recordTimezone`，confirmed-only 投影拒绝 `ai_estimate`；单位关系成立后，来源构造与响应 Schema 分别按 `NUMERIC(14,4)` 的双列舍入误差验证 `displayValue/displayUnit` 与 `canonicalValue/canonicalUnit` 属于同一测量，错配不自动修复；记录时区不参与响应本地日派生，换算门禁不重跑数值范围或改写展示值；参考时刻与来源点必须是有效时刻，点不得晚于 generatedAt；本地日期必须与 occurredAt 在响应时区中一致；动作/健康来源点必须先提供有效聚合 UUID，再按 occurredAt 非递增且 UUID 唯一，相同时刻允许、重复身份不去重；动作顶层 `identity` 与首点快照一致，健康顶层和全部点使用首点 `canonicalUnit`，空序列时两个摘要均为 `null`；`hasMore:true` 要求恰好 180 个公开点，而 `hasMore:false + 180 点` 仍是合法的不可区分状态；空证据保持 null 而非零。
