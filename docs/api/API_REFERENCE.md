...
- 客户端反馈写权限以单调 generation 收据绑定 `subjectKey + itemId + revision + eventId`，区分 `idle/submitting/succeeded/failed`。新提交、主题切换与页面失效会撤销旧收据；成功还必须与精确目标一致。失败状态只保留 offline/refused/service/unknown/conflict/invalid-contract 分类，不保存服务端正文、note、reason 或请求副本。当前模块没有页面引用、持久队列或自动刷新。
- 当前主题中性展示只消费严格解析后的共享视图：空主题明确不是零，安排标为本人提交，频次/时长只描述已记录课次；状态、本人核对、资料覆盖/限制、四类精确证据计数、时域与代次/修订分开呈现。独立页面从“我的衡迹”可达，以三个 pressed 按钮逐项请求 `training.availability`、`training.recorded_frequency` 或 `training.recorded_session_duration`；一次只读取/持有当前选择，切换使旧请求、快照、失败和焦点失权，不批量预取或缓存组合画像。客户端反馈和历史仍未接入；现有页面不会自动调用写接口。
